...
import net.certware.argument.caz.diagram.part.CazDiagramUpdater;
import net.certware.argument.caz.diagram.part.CazLinkDescriptor;
import net.certware.argument.caz.diagram.part.CazNodeDescriptor;
...
public class ArgumentDiagramCanonicalEditPolicy extends CanonicalEditPolicy {
	...
	protected Set getFeaturesToSynchronize() {
		if (myFeaturesToSynchronize == null) {
			...
			myFeaturesToSynchronize.add(CazPackage.eINSTANCE
					.getArgumentDiagram_Claims());
			myFeaturesToSynchronize.add(CazPackage.eINSTANCE
	...
	protected List getSemanticChildrenList() {
		...
		List<CazNodeDescriptor> childDescriptors = CazDiagramUpdater
				.getArgumentDiagram_1000SemanticChildren(viewObject);
		for (CazNodeDescriptor d : childDescriptors) {
	...
	private boolean isMyDiagramElement(View view) {
		int visualID = CazVisualIDRegistry.getVisualID(view);
		switch (visualID) {
	...
	protected void refreshSemantic() {
		...
		for (Iterator<CazNodeDescriptor> descriptorsIterator = childDescriptors
				.iterator(); descriptorsIterator.hasNext();) {
			CazNodeDescriptor next = descriptorsIterator.next();
			String hint = CazVisualIDRegistry.getType(next.getVisualID());
		...
		for (CazNodeDescriptor next : childDescriptors) {
			String hint = CazVisualIDRegistry.getType(next.getVisualID());
	...
	private Collection<IAdaptable> refreshConnections() {
		...
		Collection<CazLinkDescriptor> linkDescriptors = collectAllLinks(
				getDiagram(), domain2NotationMap);
		Collection existingLinks = new LinkedList(getDiagram().getEdges());
		for (Iterator linksIterator = existingLinks.iterator(); linksIterator
				.hasNext();) {
			Edge nextDiagramLink = (Edge) linksIterator.next();
			int diagramLinkVisualID = CazVisualIDRegistry
			...
			for (Iterator<CazLinkDescriptor> linkDescriptorsIterator = linkDescriptors
					.iterator(); linkDescriptorsIterator.hasNext();) {
				CazLinkDescriptor nextLinkDescriptor = linkDescriptorsIterator
						.next();
	 ...
	 * @generated
	 */
	private Collection<CazLinkDescriptor> collectAllLinks(View view,
		...
		case ArgumentDiagramEditPart.VISUAL_ID: {
			if (!domain2NotationMap.containsKey(view.getElement())) {
				result.addAll(CazDiagramUpdater
		...
		case EvidenceEditPart.VISUAL_ID: {
			if (!domain2NotationMap.containsKey(view.getElement())) {
				result.addAll(CazDiagramUpdater
						.getEvidence_2002ContainedLinks(view));
			}
			if (!domain2NotationMap.containsKey(view.getElement())
					|| view.getEAnnotation("Shortcut") == null) { //$NON-NLS-1$
				domain2NotationMap.put(view.getElement(), view);
		...
		case ArgumentEditPart.VISUAL_ID: {
			if (!domain2NotationMap.containsKey(view.getElement())) {
				result.addAll(CazDiagramUpdater
						.getArgument_2003ContainedLinks(view));
			}
			if (!domain2NotationMap.containsKey(view.getElement())
					|| view.getEAnnotation("Shortcut") == null) { //$NON-NLS-1$
				domain2NotationMap.put(view.getElement(), view);
		...
		case AssumptionEditPart.VISUAL_ID: {
			if (!domain2NotationMap.containsKey(view.getElement())) {
				result.addAll(CazDiagramUpdater
						.getAssumption_2004ContainedLinks(view));
			}
			if (!domain2NotationMap.containsKey(view.getElement())
					|| view.getEAnnotation("Shortcut") == null) { //$NON-NLS-1$
				domain2NotationMap.put(view.getElement(), view);
	...
	private Collection<IAdaptable> createConnections(
			Collection<CazLinkDescriptor> linkDescriptors,
			Map<EObject, View> domain2NotationMap) {
		LinkedList<IAdaptable> adapters = new LinkedList<IAdaptable>();
		for (CazLinkDescriptor nextLinkDescriptor : linkDescriptors) {
			...
			CreateConnectionViewRequest.ConnectionViewDescriptor descriptor = new CreateConnectionViewRequest.ConnectionViewDescriptor(
					...
					CazVisualIDRegistry.getType(nextLinkDescriptor
