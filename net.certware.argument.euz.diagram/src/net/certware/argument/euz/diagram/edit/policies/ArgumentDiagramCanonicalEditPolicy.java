...
import net.certware.argument.euz.diagram.part.EuzDiagramUpdater;
import net.certware.argument.euz.diagram.part.EuzLinkDescriptor;
import net.certware.argument.euz.diagram.part.EuzNodeDescriptor;
...
public class ArgumentDiagramCanonicalEditPolicy extends CanonicalEditPolicy {
	...
	protected Set getFeaturesToSynchronize() {
		if (myFeaturesToSynchronize == null) {
			...
			myFeaturesToSynchronize.add(EuzPackage.eINSTANCE
					.getArgumentDiagram_Arguments());
			myFeaturesToSynchronize.add(EuzPackage.eINSTANCE
	...
	protected List getSemanticChildrenList() {
		...
		List<EuzNodeDescriptor> childDescriptors = EuzDiagramUpdater
				.getArgumentDiagram_1000SemanticChildren(viewObject);
		for (EuzNodeDescriptor d : childDescriptors) {
	...
	private boolean isMyDiagramElement(View view) {
		int visualID = EuzVisualIDRegistry.getVisualID(view);
		switch (visualID) {
	...
	protected void refreshSemantic() {
		...
		for (Iterator<EuzNodeDescriptor> descriptorsIterator = childDescriptors
				.iterator(); descriptorsIterator.hasNext();) {
			EuzNodeDescriptor next = descriptorsIterator.next();
			String hint = EuzVisualIDRegistry.getType(next.getVisualID());
		...
		for (EuzNodeDescriptor next : childDescriptors) {
			String hint = EuzVisualIDRegistry.getType(next.getVisualID());
	...
	private Collection<IAdaptable> refreshConnections() {
		...
		Collection<EuzLinkDescriptor> linkDescriptors = collectAllLinks(
				getDiagram(), domain2NotationMap);
		Collection existingLinks = new LinkedList(getDiagram().getEdges());
		for (Iterator linksIterator = existingLinks.iterator(); linksIterator
				.hasNext();) {
			Edge nextDiagramLink = (Edge) linksIterator.next();
			int diagramLinkVisualID = EuzVisualIDRegistry
			...
			for (Iterator<EuzLinkDescriptor> linkDescriptorsIterator = linkDescriptors
					.iterator(); linkDescriptorsIterator.hasNext();) {
				EuzLinkDescriptor nextLinkDescriptor = linkDescriptorsIterator
						.next();
	 ...
	 * @generated
	 */
	private Collection<EuzLinkDescriptor> collectAllLinks(View view,
		...
		case ArgumentDiagramEditPart.VISUAL_ID: {
			if (!domain2NotationMap.containsKey(view.getElement())) {
				result.addAll(EuzDiagramUpdater
		...
		case StrategyEditPart.VISUAL_ID: {
			if (!domain2NotationMap.containsKey(view.getElement())) {
				result.addAll(EuzDiagramUpdater
						.getStrategy_2004ContainedLinks(view));
			}
			if (!domain2NotationMap.containsKey(view.getElement())
					|| view.getEAnnotation("Shortcut") == null) { //$NON-NLS-1$
				domain2NotationMap.put(view.getElement(), view);
		...
		case AssumptionEditPart.VISUAL_ID: {
			if (!domain2NotationMap.containsKey(view.getElement())) {
				result.addAll(EuzDiagramUpdater
						.getAssumption_2005ContainedLinks(view));
			}
			if (!domain2NotationMap.containsKey(view.getElement())
					|| view.getEAnnotation("Shortcut") == null) { //$NON-NLS-1$
				domain2NotationMap.put(view.getElement(), view);
		...
		case ContextEditPart.VISUAL_ID: {
			if (!domain2NotationMap.containsKey(view.getElement())) {
				result.addAll(EuzDiagramUpdater
						.getContext_2007ContainedLinks(view));
			}
			if (!domain2NotationMap.containsKey(view.getElement())
					|| view.getEAnnotation("Shortcut") == null) { //$NON-NLS-1$
				domain2NotationMap.put(view.getElement(), view);
		...
		case CriteriaEditPart.VISUAL_ID: {
			if (!domain2NotationMap.containsKey(view.getElement())) {
				result.addAll(EuzDiagramUpdater
						.getCriteria_2008ContainedLinks(view));
			}
			if (!domain2NotationMap.containsKey(view.getElement())
					|| view.getEAnnotation("Shortcut") == null) { //$NON-NLS-1$
				domain2NotationMap.put(view.getElement(), view);
	...
	private Collection<IAdaptable> createConnections(
			Collection<EuzLinkDescriptor> linkDescriptors,
			Map<EObject, View> domain2NotationMap) {
		LinkedList<IAdaptable> adapters = new LinkedList<IAdaptable>();
		for (EuzLinkDescriptor nextLinkDescriptor : linkDescriptors) {
			...
			CreateConnectionViewRequest.ConnectionViewDescriptor descriptor = new CreateConnectionViewRequest.ConnectionViewDescriptor(
					...
					EuzVisualIDRegistry.getType(nextLinkDescriptor
