...
public class AssumptionItemSemanticEditPolicy extends
	...
	public AssumptionItemSemanticEditPolicy() {
		super(EuzElementTypes.Assumption_2005);
	}
	...
	protected Command getDestroyElementCommand(DestroyElementRequest req) {
		...
		for (Iterator<?> it = view.getTargetEdges().iterator(); it.hasNext();) {
			...
			if (EuzVisualIDRegistry.getVisualID(incomingLink) == ArgumentArgumentAssumptionsEditPart.VISUAL_ID) {
			...
			if (EuzVisualIDRegistry.getVisualID(incomingLink) == ContextContextAssumptionsEditPart.VISUAL_ID) {
				DestroyReferenceRequest r = new DestroyReferenceRequest(
						incomingLink.getSource().getElement(), null,
						incomingLink.getTarget().getElement(), false);
				cmd.add(new DestroyReferenceCommand(r));
			...
			if (EuzVisualIDRegistry.getVisualID(incomingLink) == CriteriaCriteriaAssumptionsEditPart.VISUAL_ID) {
				DestroyReferenceRequest r = new DestroyReferenceRequest(
						incomingLink.getSource().getElement(), null,
						incomingLink.getTarget().getElement(), false);
				cmd.add(new DestroyReferenceCommand(r));
			...
			if (EuzVisualIDRegistry.getVisualID(incomingLink) == StrategyStrategyAssumptionsEditPart.VISUAL_ID) {
				DestroyReferenceRequest r = new DestroyReferenceRequest(
						incomingLink.getSource().getElement(), null,
						incomingLink.getTarget().getElement(), false);
				cmd.add(new DestroyReferenceCommand(r));
