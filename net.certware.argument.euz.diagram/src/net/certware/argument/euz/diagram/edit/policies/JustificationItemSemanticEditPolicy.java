...
public class JustificationItemSemanticEditPolicy extends
	...
	public JustificationItemSemanticEditPolicy() {
		super(EuzElementTypes.Justification_2006);
	}
	...
	protected Command getDestroyElementCommand(DestroyElementRequest req) {
		...
		for (Iterator<?> it = view.getTargetEdges().iterator(); it.hasNext();) {
			...
			if (EuzVisualIDRegistry.getVisualID(incomingLink) == ArgumentArgumentJustificationsEditPart.VISUAL_ID) {
			...
			if (EuzVisualIDRegistry.getVisualID(incomingLink) == StrategyStrategyJustificationsEditPart.VISUAL_ID) {
				DestroyReferenceRequest r = new DestroyReferenceRequest(
						incomingLink.getSource().getElement(), null,
						incomingLink.getTarget().getElement(), false);
				cmd.add(new DestroyReferenceCommand(r));
