...
import net.certware.argument.gsn.diagram.part.GsnDiagramUpdater;
import net.certware.argument.gsn.diagram.part.GsnLinkDescriptor;
import net.certware.argument.gsn.diagram.part.GsnNodeDescriptor;
...
public class ArgumentDiagramCanonicalEditPolicy extends CanonicalEditPolicy {
	...
	protected boolean isOrphaned(Collection<EObject> semanticChildren,
		...
		if (view.getEAnnotation("Shortcut") != null) { //$NON-NLS-1$
			return GsnDiagramUpdater.isShortcutOrphaned(view);
		}
	...
	private boolean isMyDiagramElement(View view) {
		...
		return visualID == GoalEditPart.VISUAL_ID
				|| visualID == JustificationEditPart.VISUAL_ID;
	}
	...
	protected void refreshSemantic() {
		...
		for (View v : getViewChildren()) {
			...
			if (v.getEAnnotation("Shortcut") != null && GsnDiagramUpdater.isShortcutOrphaned(v)) { //$NON-NLS-1$
				orphaned.add(v);
			}
		}
		...
		for (Iterator<GsnNodeDescriptor> descriptorsIterator = childDescriptors
	...
	private Collection<IAdaptable> refreshConnections() {
		...
		Collection<GsnLinkDescriptor> linkDescriptors = collectAllLinks(
				getDiagram(), domain2NotationMap);
		Collection existingLinks = new LinkedList(getDiagram().getEdges());
		for (Iterator linksIterator = existingLinks.iterator(); linksIterator
				.hasNext();) {
			Edge nextDiagramLink = (Edge) linksIterator.next();
			int diagramLinkVisualID = GsnVisualIDRegistry
			...
			for (Iterator<GsnLinkDescriptor> linkDescriptorsIterator = linkDescriptors
				...
				GsnLinkDescriptor nextLinkDescriptor = linkDescriptorsIterator
						.next();
	 ...
	 * @generated
	 */
	private Collection<GsnLinkDescriptor> collectAllLinks(View view,
		...
		case ArgumentDiagramEditPart.VISUAL_ID: {
			if (!domain2NotationMap.containsKey(view.getElement())) {
				result.addAll(GsnDiagramUpdater
		...
		case JustificationEditPart.VISUAL_ID: {
			if (!domain2NotationMap.containsKey(view.getElement())) {
				result.addAll(GsnDiagramUpdater
						.getJustification_2008ContainedLinks(view));
			}
			if (!domain2NotationMap.containsKey(view.getElement())
					|| view.getEAnnotation("Shortcut") == null) { //$NON-NLS-1$
				domain2NotationMap.put(view.getElement(), view);
	...
	private Collection<IAdaptable> createConnections(
			Collection<GsnLinkDescriptor> linkDescriptors,
			Map<EObject, View> domain2NotationMap) {
		...
		for (GsnLinkDescriptor nextLinkDescriptor : linkDescriptors) {
			...
			CreateConnectionViewRequest.ConnectionViewDescriptor descriptor = new CreateConnectionViewRequest.ConnectionViewDescriptor(
					...
					GsnVisualIDRegistry.getType(nextLinkDescriptor
