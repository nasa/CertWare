# CertWare Implementation and Integration Notes
...
Run the built-in Javadoc from Eclipse workbench with the standard doclet.  Use **Project->Generate Javadoc** to start the wizard.  Select all projects in the workspace except perhaps the test fragments and library plug-ins.  Using additional VM and doclet tag settings as below:
...
The L language editor and solver include some LaTeX-sourced documentation in the ```net.certware.doc``` plugin.  For these use a basic LaTeX installation in that source directory (in order to find the referenced figures) to produce the DVI and PDF files.  

### Web Site

The ```gh-pages``` branch of the CertWare GitHub repository includes the original web site as hosted at GitHub.  These pages and the collateral files can be edited in the branch and pushed back to the repository in order to make changes to the project's web site.  
...
* **INTENT** An intent specification metamodel.
* **L** The L-Language for argumentation and analysis, derived from two Xtext grammars -- one for editor input and one for reading the solver output.
...
* **STPA** A hazard analysis and mitigation metamodel based on Leveson's STPA.
...
### Running the EEF Tools

We use the Eclipse Extended Editing Framework (EEF) to provide better properties sheets for the EMF model editors.  The EEF model initialization occurs from the original ```*.genmodel``` files in the model plugins.  
1. The EEF model artifacts are redirected to the ```EMF.Edit``` plugins, where a ```model``` folder was created to contain the EEF models.  
1. In the ```*.eefgen``` model created, check that the generation directory property points to a source folder (typically ```src-gen```) and that the builder identifies this as a source folder (e.g. ```Build Path -> Use as Source Folder```).  
1. Be sure to add the ```model``` folder and the ```src-gen``` folders to the plugin's ```build.properties``` selections as necessary.  
1. Add ```org.eclipse.emf.eef.runtime``` as a plugin dependency, and in its properties select the re-export dependency option so that the ```EMF.editor``` project can see it.  
1. Run the ```eefgen``` to create the new packages.
1. In the ```EMF.edit``` plugin expose all of the packages in the runtime tab.  
1. In the ```EMF.editor``` plugin add the three standard provider extensions to enable the EEF contributions (see the generated plugin fragment in the ```src-gen``` folder of the ```EMF.edit``` plugin for the class name references), then add individual extensions for the tabbed property parts for example.  
1. In the generated editor file make it implement the tabbed properties interface ```org.eclipse.ui.views.properties.tabbed.ITabbedPropertySheetPageContributor``` and implement the one method that returns the contributor ID as defined in the plugin extension ```org.eclipse.ui.views.properties.tabbed.propertyContributor```.  
1. Change the ```PropertySheetPage``` reference type to ```TabbedPropertySheetPage``` and modify the ```getPropertySheetPage()``` method to work with the tabbed property sheet.  
1. Add the double-click listener on the selection viewer.    

### Running the GMF Tools
...
1. Apply the Eugenia metadata annotations to the features in the desired Ecore metamodel files (e.g. add the tags to the ```gsn.ecore``` file).
1. From the ```genmodel``` editor and root model node, run the corresponding ```genmodel``` file to create the basic EMF model, edit, and editor support source and plugins.  
1. From the explorer's context menu on the ```ecore``` file run the **Eugenia -> Generate GMF tool, graph, and map models** command.  This will yield the ```gmfgraph```, ```gmfmap```, and ```gmftool``` files next to the ```ecore``` file.  Note of the ```gmfmap``` file does not appear there is something wrong with the model structure or annotations although no error messages are produced; typical issues are container point-backs and nodes with identical tool names.
1. From the explorer's context menu on the ```gmfmap``` file run the **Create generator model command.**  This will run the wizard to create the ```gmfgen``` file.  Note that the "Load" buttons should be used on each page and that the notification that the ```genmodel``` file is out of date seems to be spurious.
1. Edit the ```gmfgen``` model's edit part properties to set Validation Enabled and Validation Decorators to ```true```.  
1. Edit the ```gmfgen``` model's edit part properties to set the metamodel Display Name property to the diagram type, e.g. **GSN Argument Diagram**.  This value appears in the property sheet header when the argument canvas is selected.
1. From the explorer's context menu on the ```gmfgen``` file run the ```Create diagram code``` command.  This will yield the ```diagram``` plugin.  Ensure this new plugin is in the run configuration or deployment package.  
...
### Running the Export Tools

Exporting to Microsoft Word format is done with ```docx4j``` and a depth-first visitor pattern.  The visitor uses the EMF collections iterators and Ecore switch generated by the model generator.  

### Running the Atlas Tools
...
The CertWare plug-ins and fragments might be delivered as collection of features, as through an update site or p2 repository, or as an Eclipse product.  
...
* ```net.certware.argument.analysis.tests``` plugin does not yet contain tests for the L language solver.  We tested the language editor and solver offline using third party tools.
* ```net.certware.argument.language.tests``` plugin does not yet contain tests for the L language grammar and editor.  We tested the language grammar and solver offline using third part tools, although this does not test the editor and workbench integration.  
...
* Prior to version 2.0 we used the **Q7** tool to perform automated testing of the GUIs.  This disappeared when the vendor and license model changed and we retired the testing project.  
