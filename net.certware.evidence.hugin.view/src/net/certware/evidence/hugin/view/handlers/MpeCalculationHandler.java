...
import net.certware.evidence.hugin.view.jobs.MpeCalculationJob;
...
/**
 * Command handler for running MPE calculation on selected network and variables.
 * @author mrb
 * @since 1.2.1
 */
public class MpeCalculationHandler extends AbstractHandler
{
  /** job name */
  protected static final String JOB_NAME = "Compute MPE values";
  
  /**
   * Handles the MPE command request.  
  ...
  public Object execute(ExecutionEvent event) throws ExecutionException
  {
    ...
    if ( network == null ) {
    	CertWareLog.logWarning("No network selected for MPE calculation");
        return null;
    }
    
    if ( view.hasVariableSelections() == false ) {
    	CertWareLog.logWarning("No variable selections for MPE calculation");
        return null;
    }

    if ( view.hasVariableStateSelections() == false ) {
    	CertWareLog.logWarning("No variable state selections for MPE calculation evidence");
        return null;
    }
    ...
    if ( nodes == null ) {
    	...
    	CertWareLog.logWarning("No variable nodes for MPE calculation");
        return null;
    }


    // perform execution in a job
    MpeCalculationJob job = new MpeCalculationJob(JOB_NAME, view);
