...
public class ArtifactCommitPropertiesEditionComponent extends SinglePartPropertiesEditingComponent {
	...
	public void initPart(Object key, int kind, EObject elt, ResourceSet allResource) {
		...
		if (editingPart != null && key == partKey) {
			...
			if (artifactCommit.getCommitIdentifier() != null && isAccessible(ScoViewsRepository.ArtifactCommit.Properties.commitIdentifier))
				basePart.setCommitIdentifier(EEFConverterUtil.convertToString(EcorePackage.eINSTANCE.getEString(), artifactCommit.getCommitIdentifier()));
			
			if (isAccessible(ScoViewsRepository.ArtifactCommit.Properties.usageTime)) {
				basePart.setUsageTime(EEFConverterUtil.convertToString(EcorePackage.eINSTANCE.getEDouble(), artifactCommit.getUsageTime()));
			}
	...
	public void updateSemanticModel(final IPropertiesEditionEvent event) {
		...
		if (ScoViewsRepository.ArtifactCommit.Properties.commitIdentifier == event.getAffectedEditor()) {
			artifactCommit.setCommitIdentifier((java.lang.String)EEFConverterUtil.createFromString(EcorePackage.eINSTANCE.getEString(), (String)event.getNewValue()));
		}
		if (ScoViewsRepository.ArtifactCommit.Properties.usageTime == event.getAffectedEditor()) {
			artifactCommit.setUsageTime((EEFConverterUtil.createDoubleFromString(EcorePackage.eINSTANCE.getEDouble(), (String)event.getNewValue())));
		}
	}
	...
	public void updatePart(Notification msg) {
		if (editingPart.isVisible()) {	
			...
			if (ScoPackage.eINSTANCE.getArtifactCommit_CommitIdentifier().equals(msg.getFeature()) && basePart != null && isAccessible(ScoViewsRepository.ArtifactCommit.Properties.commitIdentifier)) {
				if (msg.getNewValue() != null) {
					basePart.setCommitIdentifier(EcoreUtil.convertToString(EcorePackage.eINSTANCE.getEString(), msg.getNewValue()));
				} else {
					basePart.setCommitIdentifier("");
				}
			}
			if (ScoPackage.eINSTANCE.getArtifactCommit_UsageTime().equals(msg.getFeature()) && basePart != null && isAccessible(ScoViewsRepository.ArtifactCommit.Properties.usageTime)) {
				if (msg.getNewValue() != null) {
					basePart.setUsageTime(EcoreUtil.convertToString(EcorePackage.eINSTANCE.getEDouble(), msg.getNewValue()));
				} else {
			...
			try {
				if (ScoViewsRepository.ArtifactCommit.Properties.commitIdentifier == event.getAffectedEditor()) {
					...
					if (newValue instanceof String) {
						newValue = EcoreUtil.createFromString(ScoPackage.eINSTANCE.getArtifactCommit_CommitIdentifier().getEAttributeType(), (String)newValue);
					}
				...
				if (ScoViewsRepository.ArtifactCommit.Properties.usageTime == event.getAffectedEditor()) {
					Object newValue = event.getNewValue();
					if (newValue instanceof String) {
						newValue = EcoreUtil.createFromString(ScoPackage.eINSTANCE.getArtifactCommit_UsageTime().getEAttributeType(), (String)newValue);
					}
