 ...
 * 
 */
public class AmplifiesBasePropertiesEditionComponent extends SinglePartPropertiesEditingComponent {
	 ...
	 * 
	 */
	public AmplifiesBasePropertiesEditionComponent(PropertiesEditingContext editingContext, EObject amplifies, String editing_mode) {
		super(editingContext, amplifies, editing_mode);
		parts = new String[] { BASE_PART };
		repositoryKey = EvidenceViewsRepository.class;
		partKey = EvidenceViewsRepository.Amplifies.class;
	}
	...
	public void initPart(Object key, int kind, EObject elt, ResourceSet allResource) {
		...
		if (editingPart != null && key == partKey) {
			...
			final Amplifies amplifies = (Amplifies)elt;
			final AmplifiesPropertiesEditionPart basePart = (AmplifiesPropertiesEditionPart)editingPart;
			// init values
			if (isAccessible(EvidenceViewsRepository.Amplifies.Properties.taggedValue)) {
				taggedValueSettings = new ReferencesTableSettings(amplifies, SACMPackage.eINSTANCE.getModelElement_TaggedValue());
				basePart.initTaggedValue(taggedValueSettings);
			}
			if (isAccessible(EvidenceViewsRepository.Amplifies.Properties.annotation)) {
				annotationSettings = new ReferencesTableSettings(amplifies, SACMPackage.eINSTANCE.getModelElement_Annotation());
				basePart.initAnnotation(annotationSettings);
			}
			if (isAccessible(EvidenceViewsRepository.Amplifies.Properties.id))
				basePart.setId(EEFConverterUtil.convertToString(SACMPackage.Literals.STRING, amplifies.getId()));
			
			if (isAccessible(EvidenceViewsRepository.Amplifies.Properties.timing)) {
				timingSettings = new ReferencesTableSettings(amplifies, EvidencePackage.eINSTANCE.getEvidenceElement_Timing());
				basePart.initTiming(timingSettings);
			}
			if (isAccessible(EvidenceViewsRepository.Amplifies.Properties.custody)) {
				custodySettings = new ReferencesTableSettings(amplifies, EvidencePackage.eINSTANCE.getEvidenceElement_Custody());
				basePart.initCustody(custodySettings);
			}
			if (isAccessible(EvidenceViewsRepository.Amplifies.Properties.provenance)) {
				provenanceSettings = new ReferencesTableSettings(amplifies, EvidencePackage.eINSTANCE.getEvidenceElement_Provenance());
				basePart.initProvenance(provenanceSettings);
			}
			if (isAccessible(EvidenceViewsRepository.Amplifies.Properties.event)) {
				eventSettings = new ReferencesTableSettings(amplifies, EvidencePackage.eINSTANCE.getEvidenceElement_Event());
				basePart.initEvent(eventSettings);
			}
			if (isAccessible(EvidenceViewsRepository.Amplifies.Properties.attribute)) {
				attributeSettings = new ReferencesTableSettings(amplifies, EvidencePackage.eINSTANCE.getEvidenceEvaluation_Attribute());
				basePart.initAttribute(attributeSettings);
			}
			if (isAccessible(EvidenceViewsRepository.Amplifies.Properties.subject)) {
				// init part
				subjectSettings = new EObjectFlatComboSettings(amplifies, EvidencePackage.eINSTANCE.getContributes_Subject());
				basePart.initSubject(subjectSettings);
				// set the button mode
				basePart.setSubjectButtonMode(ButtonsModeEnum.BROWSE);
			}
			if (isAccessible(EvidenceViewsRepository.Amplifies.Properties.relation)) {
				// init part
				relationSettings = new EObjectFlatComboSettings(amplifies, EvidencePackage.eINSTANCE.getContributes_Relation());
	...
	public EStructuralFeature associatedFeature(Object editorKey) {
		if (editorKey == EvidenceViewsRepository.Amplifies.Properties.taggedValue) {
			return SACMPackage.eINSTANCE.getModelElement_TaggedValue();
		}
		if (editorKey == EvidenceViewsRepository.Amplifies.Properties.annotation) {
			return SACMPackage.eINSTANCE.getModelElement_Annotation();
		}
		if (editorKey == EvidenceViewsRepository.Amplifies.Properties.id) {
			return SACMPackage.eINSTANCE.getModelElement_Id();
		}
		if (editorKey == EvidenceViewsRepository.Amplifies.Properties.timing) {
			return EvidencePackage.eINSTANCE.getEvidenceElement_Timing();
		}
		if (editorKey == EvidenceViewsRepository.Amplifies.Properties.custody) {
			return EvidencePackage.eINSTANCE.getEvidenceElement_Custody();
		}
		if (editorKey == EvidenceViewsRepository.Amplifies.Properties.provenance) {
			return EvidencePackage.eINSTANCE.getEvidenceElement_Provenance();
		}
		if (editorKey == EvidenceViewsRepository.Amplifies.Properties.event) {
			return EvidencePackage.eINSTANCE.getEvidenceElement_Event();
		}
		if (editorKey == EvidenceViewsRepository.Amplifies.Properties.attribute) {
			return EvidencePackage.eINSTANCE.getEvidenceEvaluation_Attribute();
		}
		if (editorKey == EvidenceViewsRepository.Amplifies.Properties.subject) {
			return EvidencePackage.eINSTANCE.getContributes_Subject();
		}
		if (editorKey == EvidenceViewsRepository.Amplifies.Properties.relation) {
	...
	public void updateSemanticModel(final IPropertiesEditionEvent event) {
		Amplifies amplifies = (Amplifies)semanticObject;
		if (EvidenceViewsRepository.Amplifies.Properties.taggedValue == event.getAffectedEditor()) {
		...
		if (EvidenceViewsRepository.Amplifies.Properties.annotation == event.getAffectedEditor()) {
			if (event.getKind() == PropertiesEditionEvent.ADD) {
		...
		if (EvidenceViewsRepository.Amplifies.Properties.id == event.getAffectedEditor()) {
			amplifies.setId((java.lang.String)EEFConverterUtil.createFromString(SACMPackage.Literals.STRING, (String)event.getNewValue()));
		}
		if (EvidenceViewsRepository.Amplifies.Properties.timing == event.getAffectedEditor()) {
			if (event.getKind() == PropertiesEditionEvent.ADD) {
		...
		if (EvidenceViewsRepository.Amplifies.Properties.custody == event.getAffectedEditor()) {
			if (event.getKind() == PropertiesEditionEvent.ADD) {
		...
		if (EvidenceViewsRepository.Amplifies.Properties.provenance == event.getAffectedEditor()) {
			if (event.getKind() == PropertiesEditionEvent.ADD) {
		...
		if (EvidenceViewsRepository.Amplifies.Properties.event == event.getAffectedEditor()) {
			if (event.getKind() == PropertiesEditionEvent.ADD) {
		...
		if (EvidenceViewsRepository.Amplifies.Properties.attribute == event.getAffectedEditor()) {
			if (event.getKind() == PropertiesEditionEvent.ADD) {
		...
		if (EvidenceViewsRepository.Amplifies.Properties.subject == event.getAffectedEditor()) {
		...
		if (EvidenceViewsRepository.Amplifies.Properties.relation == event.getAffectedEditor()) {
			if (event.getKind() == PropertiesEditionEvent.SET) {
	...
	public void updatePart(Notification msg) {
		...
		if (editingPart.isVisible()) {
			AmplifiesPropertiesEditionPart basePart = (AmplifiesPropertiesEditionPart)editingPart;
			if (SACMPackage.eINSTANCE.getModelElement_TaggedValue().equals(msg.getFeature()) && isAccessible(EvidenceViewsRepository.Amplifies.Properties.taggedValue))
				basePart.updateTaggedValue();
			if (SACMPackage.eINSTANCE.getModelElement_Annotation().equals(msg.getFeature()) && isAccessible(EvidenceViewsRepository.Amplifies.Properties.annotation))
				basePart.updateAnnotation();
			if (SACMPackage.eINSTANCE.getModelElement_Id().equals(msg.getFeature()) && msg.getNotifier().equals(semanticObject) && basePart != null && isAccessible(EvidenceViewsRepository.Amplifies.Properties.id)) {
			...
			if (EvidencePackage.eINSTANCE.getEvidenceElement_Timing().equals(msg.getFeature()) && isAccessible(EvidenceViewsRepository.Amplifies.Properties.timing))
				basePart.updateTiming();
			if (EvidencePackage.eINSTANCE.getEvidenceElement_Custody().equals(msg.getFeature()) && isAccessible(EvidenceViewsRepository.Amplifies.Properties.custody))
				basePart.updateCustody();
			if (EvidencePackage.eINSTANCE.getEvidenceElement_Provenance().equals(msg.getFeature()) && isAccessible(EvidenceViewsRepository.Amplifies.Properties.provenance))
				basePart.updateProvenance();
			if (EvidencePackage.eINSTANCE.getEvidenceElement_Event().equals(msg.getFeature()) && isAccessible(EvidenceViewsRepository.Amplifies.Properties.event))
				basePart.updateEvent();
			if (EvidencePackage.eINSTANCE.getEvidenceEvaluation_Attribute().equals(msg.getFeature()) && isAccessible(EvidenceViewsRepository.Amplifies.Properties.attribute))
				basePart.updateAttribute();
			if (EvidencePackage.eINSTANCE.getContributes_Subject().equals(msg.getFeature()) && basePart != null && isAccessible(EvidenceViewsRepository.Amplifies.Properties.subject))
				basePart.setSubject((EObject)msg.getNewValue());
			if (EvidencePackage.eINSTANCE.getContributes_Relation().equals(msg.getFeature()) && basePart != null && isAccessible(EvidenceViewsRepository.Amplifies.Properties.relation))
	...
	public boolean isRequired(Object key, int kind) {
		return key == EvidenceViewsRepository.Amplifies.Properties.id || key == EvidenceViewsRepository.Amplifies.Properties.subject || key == EvidenceViewsRepository.Amplifies.Properties.relation;
	}
