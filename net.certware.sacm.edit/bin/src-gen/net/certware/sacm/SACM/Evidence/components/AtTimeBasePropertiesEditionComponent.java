 ...
 * 
 */
public class AtTimeBasePropertiesEditionComponent extends SinglePartPropertiesEditingComponent {
	...
	/**
	 * Settings for dateTtime EObjectFlatComboViewer
	 */
	private EObjectFlatComboSettings dateTtimeSettings;
	 ...
	 * 
	 */
	public AtTimeBasePropertiesEditionComponent(PropertiesEditingContext editingContext, EObject atTime, String editing_mode) {
		super(editingContext, atTime, editing_mode);
		parts = new String[] { BASE_PART };
		repositoryKey = EvidenceViewsRepository.class;
		partKey = EvidenceViewsRepository.AtTime.class;
	}
	...
	public void initPart(Object key, int kind, EObject elt, ResourceSet allResource) {
		...
		if (editingPart != null && key == partKey) {
			...
			final AtTime atTime = (AtTime)elt;
			final AtTimePropertiesEditionPart basePart = (AtTimePropertiesEditionPart)editingPart;
			// init values
			if (isAccessible(EvidenceViewsRepository.AtTime.Properties.taggedValue)) {
				taggedValueSettings = new ReferencesTableSettings(atTime, SACMPackage.eINSTANCE.getModelElement_TaggedValue());
				basePart.initTaggedValue(taggedValueSettings);
			}
			if (isAccessible(EvidenceViewsRepository.AtTime.Properties.annotation)) {
				annotationSettings = new ReferencesTableSettings(atTime, SACMPackage.eINSTANCE.getModelElement_Annotation());
				basePart.initAnnotation(annotationSettings);
			}
			if (isAccessible(EvidenceViewsRepository.AtTime.Properties.id))
				basePart.setId(EEFConverterUtil.convertToString(SACMPackage.Literals.STRING, atTime.getId()));
			
			if (isAccessible(EvidenceViewsRepository.AtTime.Properties.timing)) {
				timingSettings = new ReferencesTableSettings(atTime, EvidencePackage.eINSTANCE.getEvidenceElement_Timing());
				basePart.initTiming(timingSettings);
			}
			if (isAccessible(EvidenceViewsRepository.AtTime.Properties.custody)) {
				custodySettings = new ReferencesTableSettings(atTime, EvidencePackage.eINSTANCE.getEvidenceElement_Custody());
				basePart.initCustody(custodySettings);
			}
			if (isAccessible(EvidenceViewsRepository.AtTime.Properties.provenance)) {
				provenanceSettings = new ReferencesTableSettings(atTime, EvidencePackage.eINSTANCE.getEvidenceElement_Provenance());
				basePart.initProvenance(provenanceSettings);
			}
			if (isAccessible(EvidenceViewsRepository.AtTime.Properties.event)) {
				eventSettings = new ReferencesTableSettings(atTime, EvidencePackage.eINSTANCE.getEvidenceElement_Event());
				basePart.initEvent(eventSettings);
			}
			if (isAccessible(EvidenceViewsRepository.AtTime.Properties.dateTtime)) {
				// init part
				dateTtimeSettings = new EObjectFlatComboSettings(atTime, EvidencePackage.eINSTANCE.getAtTime_DateTtime());
				basePart.initDateTtime(dateTtimeSettings);
				// set the button mode
				basePart.setDateTtimeButtonMode(ButtonsModeEnum.BROWSE);
			}
			// init filters
			if (isAccessible(EvidenceViewsRepository.AtTime.Properties.taggedValue)) {
			...
			if (isAccessible(EvidenceViewsRepository.AtTime.Properties.dateTtime)) {
				basePart.addFilterToDateTtime(new ViewerFilter() {
				...
				// Start of user code for additional businessfilters for dateTtime
				// End of user code
			}
	...
	public EStructuralFeature associatedFeature(Object editorKey) {
		if (editorKey == EvidenceViewsRepository.AtTime.Properties.taggedValue) {
			return SACMPackage.eINSTANCE.getModelElement_TaggedValue();
		}
		if (editorKey == EvidenceViewsRepository.AtTime.Properties.annotation) {
			return SACMPackage.eINSTANCE.getModelElement_Annotation();
		}
		if (editorKey == EvidenceViewsRepository.AtTime.Properties.id) {
			return SACMPackage.eINSTANCE.getModelElement_Id();
		}
		if (editorKey == EvidenceViewsRepository.AtTime.Properties.timing) {
			return EvidencePackage.eINSTANCE.getEvidenceElement_Timing();
		}
		if (editorKey == EvidenceViewsRepository.AtTime.Properties.custody) {
			return EvidencePackage.eINSTANCE.getEvidenceElement_Custody();
		}
		if (editorKey == EvidenceViewsRepository.AtTime.Properties.provenance) {
			return EvidencePackage.eINSTANCE.getEvidenceElement_Provenance();
		}
		if (editorKey == EvidenceViewsRepository.AtTime.Properties.event) {
			return EvidencePackage.eINSTANCE.getEvidenceElement_Event();
		}
		if (editorKey == EvidenceViewsRepository.AtTime.Properties.dateTtime) {
			return EvidencePackage.eINSTANCE.getAtTime_DateTtime();
		}
	...
	public void updateSemanticModel(final IPropertiesEditionEvent event) {
		AtTime atTime = (AtTime)semanticObject;
		if (EvidenceViewsRepository.AtTime.Properties.taggedValue == event.getAffectedEditor()) {
		...
		if (EvidenceViewsRepository.AtTime.Properties.annotation == event.getAffectedEditor()) {
			if (event.getKind() == PropertiesEditionEvent.ADD) {
		...
		if (EvidenceViewsRepository.AtTime.Properties.id == event.getAffectedEditor()) {
			atTime.setId((java.lang.String)EEFConverterUtil.createFromString(SACMPackage.Literals.STRING, (String)event.getNewValue()));
		}
		if (EvidenceViewsRepository.AtTime.Properties.timing == event.getAffectedEditor()) {
			if (event.getKind() == PropertiesEditionEvent.ADD) {
		...
		if (EvidenceViewsRepository.AtTime.Properties.custody == event.getAffectedEditor()) {
			if (event.getKind() == PropertiesEditionEvent.ADD) {
		...
		if (EvidenceViewsRepository.AtTime.Properties.provenance == event.getAffectedEditor()) {
			if (event.getKind() == PropertiesEditionEvent.ADD) {
		...
		if (EvidenceViewsRepository.AtTime.Properties.event == event.getAffectedEditor()) {
			if (event.getKind() == PropertiesEditionEvent.ADD) {
		...
		if (EvidenceViewsRepository.AtTime.Properties.dateTtime == event.getAffectedEditor()) {
			if (event.getKind() == PropertiesEditionEvent.SET) {
				dateTtimeSettings.setToReference((Datetime)event.getNewValue());
				...
				dateTtimeSettings.setToReference(eObject);
			}
		}
	}
	...
	public void updatePart(Notification msg) {
		...
		if (editingPart.isVisible()) {
			AtTimePropertiesEditionPart basePart = (AtTimePropertiesEditionPart)editingPart;
			if (SACMPackage.eINSTANCE.getModelElement_TaggedValue().equals(msg.getFeature()) && isAccessible(EvidenceViewsRepository.AtTime.Properties.taggedValue))
				basePart.updateTaggedValue();
			if (SACMPackage.eINSTANCE.getModelElement_Annotation().equals(msg.getFeature()) && isAccessible(EvidenceViewsRepository.AtTime.Properties.annotation))
				basePart.updateAnnotation();
			if (SACMPackage.eINSTANCE.getModelElement_Id().equals(msg.getFeature()) && msg.getNotifier().equals(semanticObject) && basePart != null && isAccessible(EvidenceViewsRepository.AtTime.Properties.id)) {
			...
			if (EvidencePackage.eINSTANCE.getEvidenceElement_Timing().equals(msg.getFeature()) && isAccessible(EvidenceViewsRepository.AtTime.Properties.timing))
				basePart.updateTiming();
			if (EvidencePackage.eINSTANCE.getEvidenceElement_Custody().equals(msg.getFeature()) && isAccessible(EvidenceViewsRepository.AtTime.Properties.custody))
				basePart.updateCustody();
			if (EvidencePackage.eINSTANCE.getEvidenceElement_Provenance().equals(msg.getFeature()) && isAccessible(EvidenceViewsRepository.AtTime.Properties.provenance))
				basePart.updateProvenance();
			if (EvidencePackage.eINSTANCE.getEvidenceElement_Event().equals(msg.getFeature()) && isAccessible(EvidenceViewsRepository.AtTime.Properties.event))
				basePart.updateEvent();
			if (EvidencePackage.eINSTANCE.getAtTime_DateTtime().equals(msg.getFeature()) && basePart != null && isAccessible(EvidenceViewsRepository.AtTime.Properties.dateTtime))
				basePart.setDateTtime((EObject)msg.getNewValue());
	...
	protected NotificationFilter[] getNotificationFilters() {
		NotificationFilter filter = new EStructuralFeatureNotificationFilter(
			...
			EvidencePackage.eINSTANCE.getAtTime_DateTtime()		);
	...
	public boolean isRequired(Object key, int kind) {
		return key == EvidenceViewsRepository.AtTime.Properties.id || key == EvidenceViewsRepository.AtTime.Properties.dateTtime;
	}
