 ...
 * 
 */
public class ConflictsBasePropertiesEditionComponent extends SinglePartPropertiesEditingComponent {
	 ...
	 * 
	 */
	public ConflictsBasePropertiesEditionComponent(PropertiesEditingContext editingContext, EObject conflicts, String editing_mode) {
		super(editingContext, conflicts, editing_mode);
		parts = new String[] { BASE_PART };
		repositoryKey = EvidenceViewsRepository.class;
		partKey = EvidenceViewsRepository.Conflicts.class;
	}
	...
	public void initPart(Object key, int kind, EObject elt, ResourceSet allResource) {
		...
		if (editingPart != null && key == partKey) {
			...
			final Conflicts conflicts = (Conflicts)elt;
			final ConflictsPropertiesEditionPart basePart = (ConflictsPropertiesEditionPart)editingPart;
			// init values
			if (isAccessible(EvidenceViewsRepository.Conflicts.Properties.taggedValue)) {
				taggedValueSettings = new ReferencesTableSettings(conflicts, SACMPackage.eINSTANCE.getModelElement_TaggedValue());
				basePart.initTaggedValue(taggedValueSettings);
			}
			if (isAccessible(EvidenceViewsRepository.Conflicts.Properties.annotation)) {
				annotationSettings = new ReferencesTableSettings(conflicts, SACMPackage.eINSTANCE.getModelElement_Annotation());
				basePart.initAnnotation(annotationSettings);
			}
			if (isAccessible(EvidenceViewsRepository.Conflicts.Properties.id))
				basePart.setId(EEFConverterUtil.convertToString(SACMPackage.Literals.STRING, conflicts.getId()));
			
			if (isAccessible(EvidenceViewsRepository.Conflicts.Properties.timing)) {
				timingSettings = new ReferencesTableSettings(conflicts, EvidencePackage.eINSTANCE.getEvidenceElement_Timing());
				basePart.initTiming(timingSettings);
			}
			if (isAccessible(EvidenceViewsRepository.Conflicts.Properties.custody)) {
				custodySettings = new ReferencesTableSettings(conflicts, EvidencePackage.eINSTANCE.getEvidenceElement_Custody());
				basePart.initCustody(custodySettings);
			}
			if (isAccessible(EvidenceViewsRepository.Conflicts.Properties.provenance)) {
				provenanceSettings = new ReferencesTableSettings(conflicts, EvidencePackage.eINSTANCE.getEvidenceElement_Provenance());
				basePart.initProvenance(provenanceSettings);
			}
			if (isAccessible(EvidenceViewsRepository.Conflicts.Properties.event)) {
				eventSettings = new ReferencesTableSettings(conflicts, EvidencePackage.eINSTANCE.getEvidenceElement_Event());
				basePart.initEvent(eventSettings);
			}
			if (isAccessible(EvidenceViewsRepository.Conflicts.Properties.attribute)) {
				attributeSettings = new ReferencesTableSettings(conflicts, EvidencePackage.eINSTANCE.getEvidenceEvaluation_Attribute());
				basePart.initAttribute(attributeSettings);
			}
			if (isAccessible(EvidenceViewsRepository.Conflicts.Properties.assertion)) {
				// init part
				assertionSettings = new EObjectFlatComboSettings(conflicts, EvidencePackage.eINSTANCE.getConflicts_Assertion());
				basePart.initAssertion(assertionSettings);
				// set the button mode
				basePart.setAssertionButtonMode(ButtonsModeEnum.BROWSE);
			}
			if (isAccessible(EvidenceViewsRepository.Conflicts.Properties.subject)) {
				// init part
				subjectSettings = new EObjectFlatComboSettings(conflicts, EvidencePackage.eINSTANCE.getConflicts_Subject());
	...
	public EStructuralFeature associatedFeature(Object editorKey) {
		if (editorKey == EvidenceViewsRepository.Conflicts.Properties.taggedValue) {
			return SACMPackage.eINSTANCE.getModelElement_TaggedValue();
		}
		if (editorKey == EvidenceViewsRepository.Conflicts.Properties.annotation) {
			return SACMPackage.eINSTANCE.getModelElement_Annotation();
		}
		if (editorKey == EvidenceViewsRepository.Conflicts.Properties.id) {
			return SACMPackage.eINSTANCE.getModelElement_Id();
		}
		if (editorKey == EvidenceViewsRepository.Conflicts.Properties.timing) {
			return EvidencePackage.eINSTANCE.getEvidenceElement_Timing();
		}
		if (editorKey == EvidenceViewsRepository.Conflicts.Properties.custody) {
			return EvidencePackage.eINSTANCE.getEvidenceElement_Custody();
		}
		if (editorKey == EvidenceViewsRepository.Conflicts.Properties.provenance) {
			return EvidencePackage.eINSTANCE.getEvidenceElement_Provenance();
		}
		if (editorKey == EvidenceViewsRepository.Conflicts.Properties.event) {
			return EvidencePackage.eINSTANCE.getEvidenceElement_Event();
		}
		if (editorKey == EvidenceViewsRepository.Conflicts.Properties.attribute) {
			return EvidencePackage.eINSTANCE.getEvidenceEvaluation_Attribute();
		}
		if (editorKey == EvidenceViewsRepository.Conflicts.Properties.assertion) {
			return EvidencePackage.eINSTANCE.getConflicts_Assertion();
		}
		if (editorKey == EvidenceViewsRepository.Conflicts.Properties.subject) {
			return EvidencePackage.eINSTANCE.getConflicts_Subject();
		}
	...
	public void updateSemanticModel(final IPropertiesEditionEvent event) {
		Conflicts conflicts = (Conflicts)semanticObject;
		if (EvidenceViewsRepository.Conflicts.Properties.taggedValue == event.getAffectedEditor()) {
		...
		if (EvidenceViewsRepository.Conflicts.Properties.annotation == event.getAffectedEditor()) {
			if (event.getKind() == PropertiesEditionEvent.ADD) {
		...
		if (EvidenceViewsRepository.Conflicts.Properties.id == event.getAffectedEditor()) {
			conflicts.setId((java.lang.String)EEFConverterUtil.createFromString(SACMPackage.Literals.STRING, (String)event.getNewValue()));
		}
		if (EvidenceViewsRepository.Conflicts.Properties.timing == event.getAffectedEditor()) {
			if (event.getKind() == PropertiesEditionEvent.ADD) {
		...
		if (EvidenceViewsRepository.Conflicts.Properties.custody == event.getAffectedEditor()) {
			if (event.getKind() == PropertiesEditionEvent.ADD) {
		...
		if (EvidenceViewsRepository.Conflicts.Properties.provenance == event.getAffectedEditor()) {
			if (event.getKind() == PropertiesEditionEvent.ADD) {
		...
		if (EvidenceViewsRepository.Conflicts.Properties.event == event.getAffectedEditor()) {
			if (event.getKind() == PropertiesEditionEvent.ADD) {
		...
		if (EvidenceViewsRepository.Conflicts.Properties.attribute == event.getAffectedEditor()) {
			if (event.getKind() == PropertiesEditionEvent.ADD) {
		...
		if (EvidenceViewsRepository.Conflicts.Properties.assertion == event.getAffectedEditor()) {
		...
		if (EvidenceViewsRepository.Conflicts.Properties.subject == event.getAffectedEditor()) {
			if (event.getKind() == PropertiesEditionEvent.SET) {
				subjectSettings.setToReference((FormalAssertion)event.getNewValue());
			} else if (event.getKind() == PropertiesEditionEvent.ADD) {
	...
	public void updatePart(Notification msg) {
		...
		if (editingPart.isVisible()) {
			ConflictsPropertiesEditionPart basePart = (ConflictsPropertiesEditionPart)editingPart;
			if (SACMPackage.eINSTANCE.getModelElement_TaggedValue().equals(msg.getFeature()) && isAccessible(EvidenceViewsRepository.Conflicts.Properties.taggedValue))
				basePart.updateTaggedValue();
			if (SACMPackage.eINSTANCE.getModelElement_Annotation().equals(msg.getFeature()) && isAccessible(EvidenceViewsRepository.Conflicts.Properties.annotation))
				basePart.updateAnnotation();
			if (SACMPackage.eINSTANCE.getModelElement_Id().equals(msg.getFeature()) && msg.getNotifier().equals(semanticObject) && basePart != null && isAccessible(EvidenceViewsRepository.Conflicts.Properties.id)) {
			...
			if (EvidencePackage.eINSTANCE.getEvidenceElement_Timing().equals(msg.getFeature()) && isAccessible(EvidenceViewsRepository.Conflicts.Properties.timing))
				basePart.updateTiming();
			if (EvidencePackage.eINSTANCE.getEvidenceElement_Custody().equals(msg.getFeature()) && isAccessible(EvidenceViewsRepository.Conflicts.Properties.custody))
				basePart.updateCustody();
			if (EvidencePackage.eINSTANCE.getEvidenceElement_Provenance().equals(msg.getFeature()) && isAccessible(EvidenceViewsRepository.Conflicts.Properties.provenance))
				basePart.updateProvenance();
			if (EvidencePackage.eINSTANCE.getEvidenceElement_Event().equals(msg.getFeature()) && isAccessible(EvidenceViewsRepository.Conflicts.Properties.event))
				basePart.updateEvent();
			if (EvidencePackage.eINSTANCE.getEvidenceEvaluation_Attribute().equals(msg.getFeature()) && isAccessible(EvidenceViewsRepository.Conflicts.Properties.attribute))
				basePart.updateAttribute();
			if (EvidencePackage.eINSTANCE.getConflicts_Assertion().equals(msg.getFeature()) && basePart != null && isAccessible(EvidenceViewsRepository.Conflicts.Properties.assertion))
				basePart.setAssertion((EObject)msg.getNewValue());
			if (EvidencePackage.eINSTANCE.getConflicts_Subject().equals(msg.getFeature()) && basePart != null && isAccessible(EvidenceViewsRepository.Conflicts.Properties.subject))
	...
	protected NotificationFilter[] getNotificationFilters() {
		NotificationFilter filter = new EStructuralFeatureNotificationFilter(
			...
			EvidencePackage.eINSTANCE.getConflicts_Assertion(),
			EvidencePackage.eINSTANCE.getConflicts_Subject()		);
	...
	public boolean isRequired(Object key, int kind) {
		return key == EvidenceViewsRepository.Conflicts.Properties.id || key == EvidenceViewsRepository.Conflicts.Properties.assertion || key == EvidenceViewsRepository.Conflicts.Properties.subject;
	}
