 ...
 * 
 */
public class ConsistencyBasePropertiesEditionComponent extends SinglePartPropertiesEditingComponent {
	 ...
	 * 
	 */
	public ConsistencyBasePropertiesEditionComponent(PropertiesEditingContext editingContext, EObject consistency, String editing_mode) {
		super(editingContext, consistency, editing_mode);
		parts = new String[] { BASE_PART };
		repositoryKey = EvidenceViewsRepository.class;
		partKey = EvidenceViewsRepository.Consistency.class;
	}
	...
	public void initPart(Object key, int kind, EObject elt, ResourceSet allResource) {
		...
		if (editingPart != null && key == partKey) {
			...
			final ConsistencyPropertiesEditionPart basePart = (ConsistencyPropertiesEditionPart)editingPart;
			// init values
			if (isAccessible(EvidenceViewsRepository.Consistency.Properties.taggedValue)) {
				taggedValueSettings = new ReferencesTableSettings(consistency, SACMPackage.eINSTANCE.getModelElement_TaggedValue());
				basePart.initTaggedValue(taggedValueSettings);
			}
			if (isAccessible(EvidenceViewsRepository.Consistency.Properties.annotation)) {
				annotationSettings = new ReferencesTableSettings(consistency, SACMPackage.eINSTANCE.getModelElement_Annotation());
				basePart.initAnnotation(annotationSettings);
			}
			if (isAccessible(EvidenceViewsRepository.Consistency.Properties.id))
				basePart.setId(EEFConverterUtil.convertToString(SACMPackage.Literals.STRING, consistency.getId()));
			
			if (isAccessible(EvidenceViewsRepository.Consistency.Properties.timing)) {
				timingSettings = new ReferencesTableSettings(consistency, EvidencePackage.eINSTANCE.getEvidenceElement_Timing());
				basePart.initTiming(timingSettings);
			}
			if (isAccessible(EvidenceViewsRepository.Consistency.Properties.custody)) {
				custodySettings = new ReferencesTableSettings(consistency, EvidencePackage.eINSTANCE.getEvidenceElement_Custody());
				basePart.initCustody(custodySettings);
			}
			if (isAccessible(EvidenceViewsRepository.Consistency.Properties.provenance)) {
				provenanceSettings = new ReferencesTableSettings(consistency, EvidencePackage.eINSTANCE.getEvidenceElement_Provenance());
				basePart.initProvenance(provenanceSettings);
			}
			if (isAccessible(EvidenceViewsRepository.Consistency.Properties.event)) {
				eventSettings = new ReferencesTableSettings(consistency, EvidencePackage.eINSTANCE.getEvidenceElement_Event());
				basePart.initEvent(eventSettings);
			}
			if (isAccessible(EvidenceViewsRepository.Consistency.Properties.value)) {
				basePart.initValue(EEFUtils.choiceOfValues(consistency, EvidencePackage.eINSTANCE.getConsistency_Value()), consistency.getValue());
			}
			// init filters
			if (isAccessible(EvidenceViewsRepository.Consistency.Properties.taggedValue)) {
	...
	public EStructuralFeature associatedFeature(Object editorKey) {
		if (editorKey == EvidenceViewsRepository.Consistency.Properties.taggedValue) {
			return SACMPackage.eINSTANCE.getModelElement_TaggedValue();
		}
		if (editorKey == EvidenceViewsRepository.Consistency.Properties.annotation) {
			return SACMPackage.eINSTANCE.getModelElement_Annotation();
		}
		if (editorKey == EvidenceViewsRepository.Consistency.Properties.id) {
			return SACMPackage.eINSTANCE.getModelElement_Id();
		}
		if (editorKey == EvidenceViewsRepository.Consistency.Properties.timing) {
			return EvidencePackage.eINSTANCE.getEvidenceElement_Timing();
		}
		if (editorKey == EvidenceViewsRepository.Consistency.Properties.custody) {
			return EvidencePackage.eINSTANCE.getEvidenceElement_Custody();
		}
		if (editorKey == EvidenceViewsRepository.Consistency.Properties.provenance) {
			return EvidencePackage.eINSTANCE.getEvidenceElement_Provenance();
		}
		if (editorKey == EvidenceViewsRepository.Consistency.Properties.event) {
			return EvidencePackage.eINSTANCE.getEvidenceElement_Event();
		}
		if (editorKey == EvidenceViewsRepository.Consistency.Properties.value) {
			return EvidencePackage.eINSTANCE.getConsistency_Value();
		}
	...
	public void updateSemanticModel(final IPropertiesEditionEvent event) {
		Consistency consistency = (Consistency)semanticObject;
		if (EvidenceViewsRepository.Consistency.Properties.taggedValue == event.getAffectedEditor()) {
		...
		if (EvidenceViewsRepository.Consistency.Properties.annotation == event.getAffectedEditor()) {
			if (event.getKind() == PropertiesEditionEvent.ADD) {
		...
		if (EvidenceViewsRepository.Consistency.Properties.id == event.getAffectedEditor()) {
			consistency.setId((java.lang.String)EEFConverterUtil.createFromString(SACMPackage.Literals.STRING, (String)event.getNewValue()));
		}
		if (EvidenceViewsRepository.Consistency.Properties.timing == event.getAffectedEditor()) {
			if (event.getKind() == PropertiesEditionEvent.ADD) {
		...
		if (EvidenceViewsRepository.Consistency.Properties.custody == event.getAffectedEditor()) {
			if (event.getKind() == PropertiesEditionEvent.ADD) {
		...
		if (EvidenceViewsRepository.Consistency.Properties.provenance == event.getAffectedEditor()) {
			if (event.getKind() == PropertiesEditionEvent.ADD) {
		...
		if (EvidenceViewsRepository.Consistency.Properties.event == event.getAffectedEditor()) {
			if (event.getKind() == PropertiesEditionEvent.ADD) {
		...
		if (EvidenceViewsRepository.Consistency.Properties.value == event.getAffectedEditor()) {
			consistency.setValue((ConsistencyLevel)event.getNewValue());
		}
	}
	...
	public void updatePart(Notification msg) {
		...
		if (editingPart.isVisible()) {
			...
			if (SACMPackage.eINSTANCE.getModelElement_TaggedValue().equals(msg.getFeature()) && isAccessible(EvidenceViewsRepository.Consistency.Properties.taggedValue))
				basePart.updateTaggedValue();
			if (SACMPackage.eINSTANCE.getModelElement_Annotation().equals(msg.getFeature()) && isAccessible(EvidenceViewsRepository.Consistency.Properties.annotation))
				basePart.updateAnnotation();
			if (SACMPackage.eINSTANCE.getModelElement_Id().equals(msg.getFeature()) && msg.getNotifier().equals(semanticObject) && basePart != null && isAccessible(EvidenceViewsRepository.Consistency.Properties.id)) {
			...
			if (EvidencePackage.eINSTANCE.getEvidenceElement_Timing().equals(msg.getFeature()) && isAccessible(EvidenceViewsRepository.Consistency.Properties.timing))
				basePart.updateTiming();
			if (EvidencePackage.eINSTANCE.getEvidenceElement_Custody().equals(msg.getFeature()) && isAccessible(EvidenceViewsRepository.Consistency.Properties.custody))
				basePart.updateCustody();
			if (EvidencePackage.eINSTANCE.getEvidenceElement_Provenance().equals(msg.getFeature()) && isAccessible(EvidenceViewsRepository.Consistency.Properties.provenance))
				basePart.updateProvenance();
			if (EvidencePackage.eINSTANCE.getEvidenceElement_Event().equals(msg.getFeature()) && isAccessible(EvidenceViewsRepository.Consistency.Properties.event))
				basePart.updateEvent();
			if (EvidencePackage.eINSTANCE.getConsistency_Value().equals(msg.getFeature()) && msg.getNotifier().equals(semanticObject) && isAccessible(EvidenceViewsRepository.Consistency.Properties.value))
	...
	protected NotificationFilter[] getNotificationFilters() {
		NotificationFilter filter = new EStructuralFeatureNotificationFilter(
			...
			EvidencePackage.eINSTANCE.getConsistency_Value()		);
	...
	public boolean isRequired(Object key, int kind) {
		return key == EvidenceViewsRepository.Consistency.Properties.id || key == EvidenceViewsRepository.Consistency.Properties.value;
	}
		...
		if (event.getNewValue() != null) {
			try {
				...
				if (EvidenceViewsRepository.Consistency.Properties.value == event.getAffectedEditor()) {
					Object newValue = event.getNewValue();
					if (newValue instanceof String) {
						newValue = EEFConverterUtil.createFromString(EvidencePackage.eINSTANCE.getConsistency_Value().getEAttributeType(), (String)newValue);
					}
					ret = Diagnostician.INSTANCE.validate(EvidencePackage.eINSTANCE.getConsistency_Value().getEAttributeType(), newValue);
				}
