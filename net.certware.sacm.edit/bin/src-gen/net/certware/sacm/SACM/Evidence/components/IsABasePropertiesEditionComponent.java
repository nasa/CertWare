 ...
 * 
 */
public class IsABasePropertiesEditionComponent extends SinglePartPropertiesEditingComponent {
	 ...
	 * 
	 */
	public IsABasePropertiesEditionComponent(PropertiesEditingContext editingContext, EObject isA, String editing_mode) {
		super(editingContext, isA, editing_mode);
		parts = new String[] { BASE_PART };
		repositoryKey = EvidenceViewsRepository.class;
		partKey = EvidenceViewsRepository.IsA.class;
	}
	...
	public void initPart(Object key, int kind, EObject elt, ResourceSet allResource) {
		...
		if (editingPart != null && key == partKey) {
			...
			final IsA isA = (IsA)elt;
			final IsAPropertiesEditionPart basePart = (IsAPropertiesEditionPart)editingPart;
			// init values
			if (isAccessible(EvidenceViewsRepository.IsA.Properties.taggedValue)) {
				taggedValueSettings = new ReferencesTableSettings(isA, SACMPackage.eINSTANCE.getModelElement_TaggedValue());
				basePart.initTaggedValue(taggedValueSettings);
			}
			if (isAccessible(EvidenceViewsRepository.IsA.Properties.annotation)) {
				annotationSettings = new ReferencesTableSettings(isA, SACMPackage.eINSTANCE.getModelElement_Annotation());
				basePart.initAnnotation(annotationSettings);
			}
			if (isAccessible(EvidenceViewsRepository.IsA.Properties.id))
				basePart.setId(EEFConverterUtil.convertToString(SACMPackage.Literals.STRING, isA.getId()));
			
			if (isAccessible(EvidenceViewsRepository.IsA.Properties.timing)) {
				timingSettings = new ReferencesTableSettings(isA, EvidencePackage.eINSTANCE.getEvidenceElement_Timing());
				basePart.initTiming(timingSettings);
			}
			if (isAccessible(EvidenceViewsRepository.IsA.Properties.custody)) {
				custodySettings = new ReferencesTableSettings(isA, EvidencePackage.eINSTANCE.getEvidenceElement_Custody());
				basePart.initCustody(custodySettings);
			}
			if (isAccessible(EvidenceViewsRepository.IsA.Properties.provenance)) {
				provenanceSettings = new ReferencesTableSettings(isA, EvidencePackage.eINSTANCE.getEvidenceElement_Provenance());
				basePart.initProvenance(provenanceSettings);
			}
			if (isAccessible(EvidenceViewsRepository.IsA.Properties.event)) {
				eventSettings = new ReferencesTableSettings(isA, EvidencePackage.eINSTANCE.getEvidenceElement_Event());
				basePart.initEvent(eventSettings);
			}
			if (isAccessible(EvidenceViewsRepository.IsA.Properties.attribute)) {
				attributeSettings = new ReferencesTableSettings(isA, EvidencePackage.eINSTANCE.getEvidenceEvaluation_Attribute());
				basePart.initAttribute(attributeSettings);
			}
			if (isAccessible(EvidenceViewsRepository.IsA.Properties.subject)) {
				// init part
				subjectSettings = new EObjectFlatComboSettings(isA, EvidencePackage.eINSTANCE.getEvidenceInterpretation_Subject());
				basePart.initSubject(subjectSettings);
				// set the button mode
				basePart.setSubjectButtonMode(ButtonsModeEnum.BROWSE);
			}
			if (isAccessible(EvidenceViewsRepository.IsA.Properties.definition)) {
				// init part
				definitionSettings = new EObjectFlatComboSettings(isA, EvidencePackage.eINSTANCE.getIsA_Definition());
	...
	public EStructuralFeature associatedFeature(Object editorKey) {
		if (editorKey == EvidenceViewsRepository.IsA.Properties.taggedValue) {
			return SACMPackage.eINSTANCE.getModelElement_TaggedValue();
		}
		if (editorKey == EvidenceViewsRepository.IsA.Properties.annotation) {
			return SACMPackage.eINSTANCE.getModelElement_Annotation();
		}
		if (editorKey == EvidenceViewsRepository.IsA.Properties.id) {
			return SACMPackage.eINSTANCE.getModelElement_Id();
		}
		if (editorKey == EvidenceViewsRepository.IsA.Properties.timing) {
			return EvidencePackage.eINSTANCE.getEvidenceElement_Timing();
		}
		if (editorKey == EvidenceViewsRepository.IsA.Properties.custody) {
			return EvidencePackage.eINSTANCE.getEvidenceElement_Custody();
		}
		if (editorKey == EvidenceViewsRepository.IsA.Properties.provenance) {
			return EvidencePackage.eINSTANCE.getEvidenceElement_Provenance();
		}
		if (editorKey == EvidenceViewsRepository.IsA.Properties.event) {
			return EvidencePackage.eINSTANCE.getEvidenceElement_Event();
		}
		if (editorKey == EvidenceViewsRepository.IsA.Properties.attribute) {
			return EvidencePackage.eINSTANCE.getEvidenceEvaluation_Attribute();
		}
		if (editorKey == EvidenceViewsRepository.IsA.Properties.subject) {
			return EvidencePackage.eINSTANCE.getEvidenceInterpretation_Subject();
		}
		if (editorKey == EvidenceViewsRepository.IsA.Properties.definition) {
			return EvidencePackage.eINSTANCE.getIsA_Definition();
		}
	...
	public void updateSemanticModel(final IPropertiesEditionEvent event) {
		IsA isA = (IsA)semanticObject;
		if (EvidenceViewsRepository.IsA.Properties.taggedValue == event.getAffectedEditor()) {
		...
		if (EvidenceViewsRepository.IsA.Properties.annotation == event.getAffectedEditor()) {
			if (event.getKind() == PropertiesEditionEvent.ADD) {
		...
		if (EvidenceViewsRepository.IsA.Properties.id == event.getAffectedEditor()) {
			isA.setId((java.lang.String)EEFConverterUtil.createFromString(SACMPackage.Literals.STRING, (String)event.getNewValue()));
		}
		if (EvidenceViewsRepository.IsA.Properties.timing == event.getAffectedEditor()) {
			if (event.getKind() == PropertiesEditionEvent.ADD) {
		...
		if (EvidenceViewsRepository.IsA.Properties.custody == event.getAffectedEditor()) {
			if (event.getKind() == PropertiesEditionEvent.ADD) {
		...
		if (EvidenceViewsRepository.IsA.Properties.provenance == event.getAffectedEditor()) {
			if (event.getKind() == PropertiesEditionEvent.ADD) {
		...
		if (EvidenceViewsRepository.IsA.Properties.event == event.getAffectedEditor()) {
			if (event.getKind() == PropertiesEditionEvent.ADD) {
		...
		if (EvidenceViewsRepository.IsA.Properties.attribute == event.getAffectedEditor()) {
			if (event.getKind() == PropertiesEditionEvent.ADD) {
		...
		if (EvidenceViewsRepository.IsA.Properties.subject == event.getAffectedEditor()) {
		...
		if (EvidenceViewsRepository.IsA.Properties.definition == event.getAffectedEditor()) {
			if (event.getKind() == PropertiesEditionEvent.SET) {
				definitionSettings.setToReference((FormalElement)event.getNewValue());
			} else if (event.getKind() == PropertiesEditionEvent.ADD) {
				EReferencePropertiesEditionContext context = new EReferencePropertiesEditionContext(editingContext, this, definitionSettings, editingContext.getAdapterFactory());
				PropertiesEditingProvider provider = (PropertiesEditingProvider)editingContext.getAdapterFactory().adapt(semanticObject, PropertiesEditingProvider.class);
				if (provider != null) {
					PropertiesEditingPolicy policy = provider.getPolicy(context);
					if (policy instanceof CreateEditingPolicy) {
	...
	public void updatePart(Notification msg) {
		...
		if (editingPart.isVisible()) {
			IsAPropertiesEditionPart basePart = (IsAPropertiesEditionPart)editingPart;
			if (SACMPackage.eINSTANCE.getModelElement_TaggedValue().equals(msg.getFeature()) && isAccessible(EvidenceViewsRepository.IsA.Properties.taggedValue))
				basePart.updateTaggedValue();
			if (SACMPackage.eINSTANCE.getModelElement_Annotation().equals(msg.getFeature()) && isAccessible(EvidenceViewsRepository.IsA.Properties.annotation))
				basePart.updateAnnotation();
			if (SACMPackage.eINSTANCE.getModelElement_Id().equals(msg.getFeature()) && msg.getNotifier().equals(semanticObject) && basePart != null && isAccessible(EvidenceViewsRepository.IsA.Properties.id)) {
			...
			if (EvidencePackage.eINSTANCE.getEvidenceElement_Timing().equals(msg.getFeature()) && isAccessible(EvidenceViewsRepository.IsA.Properties.timing))
				basePart.updateTiming();
			if (EvidencePackage.eINSTANCE.getEvidenceElement_Custody().equals(msg.getFeature()) && isAccessible(EvidenceViewsRepository.IsA.Properties.custody))
				basePart.updateCustody();
			if (EvidencePackage.eINSTANCE.getEvidenceElement_Provenance().equals(msg.getFeature()) && isAccessible(EvidenceViewsRepository.IsA.Properties.provenance))
				basePart.updateProvenance();
			if (EvidencePackage.eINSTANCE.getEvidenceElement_Event().equals(msg.getFeature()) && isAccessible(EvidenceViewsRepository.IsA.Properties.event))
				basePart.updateEvent();
			if (EvidencePackage.eINSTANCE.getEvidenceEvaluation_Attribute().equals(msg.getFeature()) && isAccessible(EvidenceViewsRepository.IsA.Properties.attribute))
				basePart.updateAttribute();
			if (EvidencePackage.eINSTANCE.getEvidenceInterpretation_Subject().equals(msg.getFeature()) && basePart != null && isAccessible(EvidenceViewsRepository.IsA.Properties.subject))
				basePart.setSubject((EObject)msg.getNewValue());
			if (EvidencePackage.eINSTANCE.getIsA_Definition().equals(msg.getFeature()) && basePart != null && isAccessible(EvidenceViewsRepository.IsA.Properties.definition))
	...
	protected NotificationFilter[] getNotificationFilters() {
		NotificationFilter filter = new EStructuralFeatureNotificationFilter(
			...
			EvidencePackage.eINSTANCE.getIsA_Definition()		);
	...
	public boolean isRequired(Object key, int kind) {
		return key == EvidenceViewsRepository.IsA.Properties.id || key == EvidenceViewsRepository.IsA.Properties.subject || key == EvidenceViewsRepository.IsA.Properties.definition;
	}
