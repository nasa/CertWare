 ...
 * 
 */
public class IsRevokedAtBasePropertiesEditionComponent extends SinglePartPropertiesEditingComponent {
	 ...
	 * 
	 */
	public IsRevokedAtBasePropertiesEditionComponent(PropertiesEditingContext editingContext, EObject isRevokedAt, String editing_mode) {
		super(editingContext, isRevokedAt, editing_mode);
		parts = new String[] { BASE_PART };
		repositoryKey = EvidenceViewsRepository.class;
		partKey = EvidenceViewsRepository.IsRevokedAt.class;
	}
	...
	public void initPart(Object key, int kind, EObject elt, ResourceSet allResource) {
		...
		if (editingPart != null && key == partKey) {
			...
			final IsRevokedAt isRevokedAt = (IsRevokedAt)elt;
			final IsRevokedAtPropertiesEditionPart basePart = (IsRevokedAtPropertiesEditionPart)editingPart;
			// init values
			if (isAccessible(EvidenceViewsRepository.IsRevokedAt.Properties.taggedValue)) {
				taggedValueSettings = new ReferencesTableSettings(isRevokedAt, SACMPackage.eINSTANCE.getModelElement_TaggedValue());
				basePart.initTaggedValue(taggedValueSettings);
			}
			if (isAccessible(EvidenceViewsRepository.IsRevokedAt.Properties.annotation)) {
				annotationSettings = new ReferencesTableSettings(isRevokedAt, SACMPackage.eINSTANCE.getModelElement_Annotation());
				basePart.initAnnotation(annotationSettings);
			}
			if (isAccessible(EvidenceViewsRepository.IsRevokedAt.Properties.id))
				basePart.setId(EEFConverterUtil.convertToString(SACMPackage.Literals.STRING, isRevokedAt.getId()));
			
			if (isAccessible(EvidenceViewsRepository.IsRevokedAt.Properties.timing)) {
				timingSettings = new ReferencesTableSettings(isRevokedAt, EvidencePackage.eINSTANCE.getEvidenceElement_Timing());
				basePart.initTiming(timingSettings);
			}
			if (isAccessible(EvidenceViewsRepository.IsRevokedAt.Properties.custody)) {
				custodySettings = new ReferencesTableSettings(isRevokedAt, EvidencePackage.eINSTANCE.getEvidenceElement_Custody());
				basePart.initCustody(custodySettings);
			}
			if (isAccessible(EvidenceViewsRepository.IsRevokedAt.Properties.provenance)) {
				provenanceSettings = new ReferencesTableSettings(isRevokedAt, EvidencePackage.eINSTANCE.getEvidenceElement_Provenance());
				basePart.initProvenance(provenanceSettings);
			}
			if (isAccessible(EvidenceViewsRepository.IsRevokedAt.Properties.event)) {
				eventSettings = new ReferencesTableSettings(isRevokedAt, EvidencePackage.eINSTANCE.getEvidenceElement_Event());
	...
	public EStructuralFeature associatedFeature(Object editorKey) {
		if (editorKey == EvidenceViewsRepository.IsRevokedAt.Properties.taggedValue) {
			return SACMPackage.eINSTANCE.getModelElement_TaggedValue();
		}
		if (editorKey == EvidenceViewsRepository.IsRevokedAt.Properties.annotation) {
			return SACMPackage.eINSTANCE.getModelElement_Annotation();
		}
		if (editorKey == EvidenceViewsRepository.IsRevokedAt.Properties.id) {
			return SACMPackage.eINSTANCE.getModelElement_Id();
		}
		if (editorKey == EvidenceViewsRepository.IsRevokedAt.Properties.timing) {
			return EvidencePackage.eINSTANCE.getEvidenceElement_Timing();
		}
		if (editorKey == EvidenceViewsRepository.IsRevokedAt.Properties.custody) {
			return EvidencePackage.eINSTANCE.getEvidenceElement_Custody();
		}
		if (editorKey == EvidenceViewsRepository.IsRevokedAt.Properties.provenance) {
			return EvidencePackage.eINSTANCE.getEvidenceElement_Provenance();
		}
		if (editorKey == EvidenceViewsRepository.IsRevokedAt.Properties.event) {
	...
	public void updateSemanticModel(final IPropertiesEditionEvent event) {
		IsRevokedAt isRevokedAt = (IsRevokedAt)semanticObject;
		if (EvidenceViewsRepository.IsRevokedAt.Properties.taggedValue == event.getAffectedEditor()) {
		...
		if (EvidenceViewsRepository.IsRevokedAt.Properties.annotation == event.getAffectedEditor()) {
			if (event.getKind() == PropertiesEditionEvent.ADD) {
		...
		if (EvidenceViewsRepository.IsRevokedAt.Properties.id == event.getAffectedEditor()) {
			isRevokedAt.setId((java.lang.String)EEFConverterUtil.createFromString(SACMPackage.Literals.STRING, (String)event.getNewValue()));
		}
		if (EvidenceViewsRepository.IsRevokedAt.Properties.timing == event.getAffectedEditor()) {
			if (event.getKind() == PropertiesEditionEvent.ADD) {
		...
		if (EvidenceViewsRepository.IsRevokedAt.Properties.custody == event.getAffectedEditor()) {
			if (event.getKind() == PropertiesEditionEvent.ADD) {
		...
		if (EvidenceViewsRepository.IsRevokedAt.Properties.provenance == event.getAffectedEditor()) {
			if (event.getKind() == PropertiesEditionEvent.ADD) {
		...
		if (EvidenceViewsRepository.IsRevokedAt.Properties.event == event.getAffectedEditor()) {
			if (event.getKind() == PropertiesEditionEvent.ADD) {
	...
	public void updatePart(Notification msg) {
		...
		if (editingPart.isVisible()) {
			IsRevokedAtPropertiesEditionPart basePart = (IsRevokedAtPropertiesEditionPart)editingPart;
			if (SACMPackage.eINSTANCE.getModelElement_TaggedValue().equals(msg.getFeature()) && isAccessible(EvidenceViewsRepository.IsRevokedAt.Properties.taggedValue))
				basePart.updateTaggedValue();
			if (SACMPackage.eINSTANCE.getModelElement_Annotation().equals(msg.getFeature()) && isAccessible(EvidenceViewsRepository.IsRevokedAt.Properties.annotation))
				basePart.updateAnnotation();
			if (SACMPackage.eINSTANCE.getModelElement_Id().equals(msg.getFeature()) && msg.getNotifier().equals(semanticObject) && basePart != null && isAccessible(EvidenceViewsRepository.IsRevokedAt.Properties.id)) {
			...
			if (EvidencePackage.eINSTANCE.getEvidenceElement_Timing().equals(msg.getFeature()) && isAccessible(EvidenceViewsRepository.IsRevokedAt.Properties.timing))
				basePart.updateTiming();
			if (EvidencePackage.eINSTANCE.getEvidenceElement_Custody().equals(msg.getFeature()) && isAccessible(EvidenceViewsRepository.IsRevokedAt.Properties.custody))
				basePart.updateCustody();
			if (EvidencePackage.eINSTANCE.getEvidenceElement_Provenance().equals(msg.getFeature()) && isAccessible(EvidenceViewsRepository.IsRevokedAt.Properties.provenance))
				basePart.updateProvenance();
			if (EvidencePackage.eINSTANCE.getEvidenceElement_Event().equals(msg.getFeature()) && isAccessible(EvidenceViewsRepository.IsRevokedAt.Properties.event))
	...
	public boolean isRequired(Object key, int kind) {
		return key == EvidenceViewsRepository.IsRevokedAt.Properties.id;
	}
