 ...
 * 
 */
public class ProvidesContextBasePropertiesEditionComponent extends SinglePartPropertiesEditingComponent {
	 ...
	 * 
	 */
	public ProvidesContextBasePropertiesEditionComponent(PropertiesEditingContext editingContext, EObject providesContext, String editing_mode) {
		super(editingContext, providesContext, editing_mode);
		parts = new String[] { BASE_PART };
		repositoryKey = EvidenceViewsRepository.class;
		partKey = EvidenceViewsRepository.ProvidesContext.class;
	}
	...
	public void initPart(Object key, int kind, EObject elt, ResourceSet allResource) {
		...
		if (editingPart != null && key == partKey) {
			...
			final ProvidesContext providesContext = (ProvidesContext)elt;
			final ProvidesContextPropertiesEditionPart basePart = (ProvidesContextPropertiesEditionPart)editingPart;
			// init values
			if (isAccessible(EvidenceViewsRepository.ProvidesContext.Properties.taggedValue)) {
				taggedValueSettings = new ReferencesTableSettings(providesContext, SACMPackage.eINSTANCE.getModelElement_TaggedValue());
				basePart.initTaggedValue(taggedValueSettings);
			}
			if (isAccessible(EvidenceViewsRepository.ProvidesContext.Properties.annotation)) {
				annotationSettings = new ReferencesTableSettings(providesContext, SACMPackage.eINSTANCE.getModelElement_Annotation());
				basePart.initAnnotation(annotationSettings);
			}
			if (isAccessible(EvidenceViewsRepository.ProvidesContext.Properties.id))
				basePart.setId(EEFConverterUtil.convertToString(SACMPackage.Literals.STRING, providesContext.getId()));
			
			if (isAccessible(EvidenceViewsRepository.ProvidesContext.Properties.timing)) {
				timingSettings = new ReferencesTableSettings(providesContext, EvidencePackage.eINSTANCE.getEvidenceElement_Timing());
				basePart.initTiming(timingSettings);
			}
			if (isAccessible(EvidenceViewsRepository.ProvidesContext.Properties.custody)) {
				custodySettings = new ReferencesTableSettings(providesContext, EvidencePackage.eINSTANCE.getEvidenceElement_Custody());
				basePart.initCustody(custodySettings);
			}
			if (isAccessible(EvidenceViewsRepository.ProvidesContext.Properties.provenance)) {
				provenanceSettings = new ReferencesTableSettings(providesContext, EvidencePackage.eINSTANCE.getEvidenceElement_Provenance());
				basePart.initProvenance(provenanceSettings);
			}
			if (isAccessible(EvidenceViewsRepository.ProvidesContext.Properties.event)) {
				eventSettings = new ReferencesTableSettings(providesContext, EvidencePackage.eINSTANCE.getEvidenceElement_Event());
				basePart.initEvent(eventSettings);
			}
			if (isAccessible(EvidenceViewsRepository.ProvidesContext.Properties.attribute)) {
				attributeSettings = new ReferencesTableSettings(providesContext, EvidencePackage.eINSTANCE.getEvidenceEvaluation_Attribute());
				basePart.initAttribute(attributeSettings);
			}
			if (isAccessible(EvidenceViewsRepository.ProvidesContext.Properties.subject)) {
				// init part
				subjectSettings = new EObjectFlatComboSettings(providesContext, EvidencePackage.eINSTANCE.getEvidenceInterpretation_Subject());
				basePart.initSubject(subjectSettings);
				// set the button mode
				basePart.setSubjectButtonMode(ButtonsModeEnum.BROWSE);
			}
			if (isAccessible(EvidenceViewsRepository.ProvidesContext.Properties.context)) {
				// init part
				contextSettings = new EObjectFlatComboSettings(providesContext, EvidencePackage.eINSTANCE.getProvidesContext_Context());
	...
	public EStructuralFeature associatedFeature(Object editorKey) {
		if (editorKey == EvidenceViewsRepository.ProvidesContext.Properties.taggedValue) {
			return SACMPackage.eINSTANCE.getModelElement_TaggedValue();
		}
		if (editorKey == EvidenceViewsRepository.ProvidesContext.Properties.annotation) {
			return SACMPackage.eINSTANCE.getModelElement_Annotation();
		}
		if (editorKey == EvidenceViewsRepository.ProvidesContext.Properties.id) {
			return SACMPackage.eINSTANCE.getModelElement_Id();
		}
		if (editorKey == EvidenceViewsRepository.ProvidesContext.Properties.timing) {
			return EvidencePackage.eINSTANCE.getEvidenceElement_Timing();
		}
		if (editorKey == EvidenceViewsRepository.ProvidesContext.Properties.custody) {
			return EvidencePackage.eINSTANCE.getEvidenceElement_Custody();
		}
		if (editorKey == EvidenceViewsRepository.ProvidesContext.Properties.provenance) {
			return EvidencePackage.eINSTANCE.getEvidenceElement_Provenance();
		}
		if (editorKey == EvidenceViewsRepository.ProvidesContext.Properties.event) {
			return EvidencePackage.eINSTANCE.getEvidenceElement_Event();
		}
		if (editorKey == EvidenceViewsRepository.ProvidesContext.Properties.attribute) {
			return EvidencePackage.eINSTANCE.getEvidenceEvaluation_Attribute();
		}
		if (editorKey == EvidenceViewsRepository.ProvidesContext.Properties.subject) {
			return EvidencePackage.eINSTANCE.getEvidenceInterpretation_Subject();
		}
		if (editorKey == EvidenceViewsRepository.ProvidesContext.Properties.context) {
			return EvidencePackage.eINSTANCE.getProvidesContext_Context();
		}
	...
	public void updateSemanticModel(final IPropertiesEditionEvent event) {
		ProvidesContext providesContext = (ProvidesContext)semanticObject;
		if (EvidenceViewsRepository.ProvidesContext.Properties.taggedValue == event.getAffectedEditor()) {
		...
		if (EvidenceViewsRepository.ProvidesContext.Properties.annotation == event.getAffectedEditor()) {
			if (event.getKind() == PropertiesEditionEvent.ADD) {
		...
		if (EvidenceViewsRepository.ProvidesContext.Properties.id == event.getAffectedEditor()) {
			providesContext.setId((java.lang.String)EEFConverterUtil.createFromString(SACMPackage.Literals.STRING, (String)event.getNewValue()));
		}
		if (EvidenceViewsRepository.ProvidesContext.Properties.timing == event.getAffectedEditor()) {
			if (event.getKind() == PropertiesEditionEvent.ADD) {
		...
		if (EvidenceViewsRepository.ProvidesContext.Properties.custody == event.getAffectedEditor()) {
			if (event.getKind() == PropertiesEditionEvent.ADD) {
		...
		if (EvidenceViewsRepository.ProvidesContext.Properties.provenance == event.getAffectedEditor()) {
			if (event.getKind() == PropertiesEditionEvent.ADD) {
		...
		if (EvidenceViewsRepository.ProvidesContext.Properties.event == event.getAffectedEditor()) {
			if (event.getKind() == PropertiesEditionEvent.ADD) {
		...
		if (EvidenceViewsRepository.ProvidesContext.Properties.attribute == event.getAffectedEditor()) {
			if (event.getKind() == PropertiesEditionEvent.ADD) {
		...
		if (EvidenceViewsRepository.ProvidesContext.Properties.subject == event.getAffectedEditor()) {
		...
		if (EvidenceViewsRepository.ProvidesContext.Properties.context == event.getAffectedEditor()) {
			if (event.getKind() == PropertiesEditionEvent.SET) {
				contextSettings.setToReference((EvidenceElement)event.getNewValue());
			} else if (event.getKind() == PropertiesEditionEvent.ADD) {
	...
	public void updatePart(Notification msg) {
		...
		if (editingPart.isVisible()) {
			ProvidesContextPropertiesEditionPart basePart = (ProvidesContextPropertiesEditionPart)editingPart;
			if (SACMPackage.eINSTANCE.getModelElement_TaggedValue().equals(msg.getFeature()) && isAccessible(EvidenceViewsRepository.ProvidesContext.Properties.taggedValue))
				basePart.updateTaggedValue();
			if (SACMPackage.eINSTANCE.getModelElement_Annotation().equals(msg.getFeature()) && isAccessible(EvidenceViewsRepository.ProvidesContext.Properties.annotation))
				basePart.updateAnnotation();
			if (SACMPackage.eINSTANCE.getModelElement_Id().equals(msg.getFeature()) && msg.getNotifier().equals(semanticObject) && basePart != null && isAccessible(EvidenceViewsRepository.ProvidesContext.Properties.id)) {
			...
			if (EvidencePackage.eINSTANCE.getEvidenceElement_Timing().equals(msg.getFeature()) && isAccessible(EvidenceViewsRepository.ProvidesContext.Properties.timing))
				basePart.updateTiming();
			if (EvidencePackage.eINSTANCE.getEvidenceElement_Custody().equals(msg.getFeature()) && isAccessible(EvidenceViewsRepository.ProvidesContext.Properties.custody))
				basePart.updateCustody();
			if (EvidencePackage.eINSTANCE.getEvidenceElement_Provenance().equals(msg.getFeature()) && isAccessible(EvidenceViewsRepository.ProvidesContext.Properties.provenance))
				basePart.updateProvenance();
			if (EvidencePackage.eINSTANCE.getEvidenceElement_Event().equals(msg.getFeature()) && isAccessible(EvidenceViewsRepository.ProvidesContext.Properties.event))
				basePart.updateEvent();
			if (EvidencePackage.eINSTANCE.getEvidenceEvaluation_Attribute().equals(msg.getFeature()) && isAccessible(EvidenceViewsRepository.ProvidesContext.Properties.attribute))
				basePart.updateAttribute();
			if (EvidencePackage.eINSTANCE.getEvidenceInterpretation_Subject().equals(msg.getFeature()) && basePart != null && isAccessible(EvidenceViewsRepository.ProvidesContext.Properties.subject))
				basePart.setSubject((EObject)msg.getNewValue());
			if (EvidencePackage.eINSTANCE.getProvidesContext_Context().equals(msg.getFeature()) && basePart != null && isAccessible(EvidenceViewsRepository.ProvidesContext.Properties.context))
	...
	protected NotificationFilter[] getNotificationFilters() {
		NotificationFilter filter = new EStructuralFeatureNotificationFilter(
			...
			EvidencePackage.eINSTANCE.getProvidesContext_Context()		);
	...
	public boolean isRequired(Object key, int kind) {
		return key == EvidenceViewsRepository.ProvidesContext.Properties.id || key == EvidenceViewsRepository.ProvidesContext.Properties.subject || key == EvidenceViewsRepository.ProvidesContext.Properties.context;
	}
