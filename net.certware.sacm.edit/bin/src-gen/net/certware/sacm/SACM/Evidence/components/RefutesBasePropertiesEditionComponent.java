 ...
 * 
 */
public class RefutesBasePropertiesEditionComponent extends SinglePartPropertiesEditingComponent {
	 ...
	 * 
	 */
	public RefutesBasePropertiesEditionComponent(PropertiesEditingContext editingContext, EObject refutes, String editing_mode) {
		super(editingContext, refutes, editing_mode);
		parts = new String[] { BASE_PART };
		repositoryKey = EvidenceViewsRepository.class;
		partKey = EvidenceViewsRepository.Refutes.class;
	}
	...
	public void initPart(Object key, int kind, EObject elt, ResourceSet allResource) {
		...
		if (editingPart != null && key == partKey) {
			...
			final Refutes refutes = (Refutes)elt;
			final RefutesPropertiesEditionPart basePart = (RefutesPropertiesEditionPart)editingPart;
			// init values
			if (isAccessible(EvidenceViewsRepository.Refutes.Properties.taggedValue)) {
				taggedValueSettings = new ReferencesTableSettings(refutes, SACMPackage.eINSTANCE.getModelElement_TaggedValue());
				basePart.initTaggedValue(taggedValueSettings);
			}
			if (isAccessible(EvidenceViewsRepository.Refutes.Properties.annotation)) {
				annotationSettings = new ReferencesTableSettings(refutes, SACMPackage.eINSTANCE.getModelElement_Annotation());
				basePart.initAnnotation(annotationSettings);
			}
			if (isAccessible(EvidenceViewsRepository.Refutes.Properties.id))
				basePart.setId(EEFConverterUtil.convertToString(SACMPackage.Literals.STRING, refutes.getId()));
			
			if (isAccessible(EvidenceViewsRepository.Refutes.Properties.timing)) {
				timingSettings = new ReferencesTableSettings(refutes, EvidencePackage.eINSTANCE.getEvidenceElement_Timing());
				basePart.initTiming(timingSettings);
			}
			if (isAccessible(EvidenceViewsRepository.Refutes.Properties.custody)) {
				custodySettings = new ReferencesTableSettings(refutes, EvidencePackage.eINSTANCE.getEvidenceElement_Custody());
				basePart.initCustody(custodySettings);
			}
			if (isAccessible(EvidenceViewsRepository.Refutes.Properties.provenance)) {
				provenanceSettings = new ReferencesTableSettings(refutes, EvidencePackage.eINSTANCE.getEvidenceElement_Provenance());
				basePart.initProvenance(provenanceSettings);
			}
			if (isAccessible(EvidenceViewsRepository.Refutes.Properties.event)) {
				eventSettings = new ReferencesTableSettings(refutes, EvidencePackage.eINSTANCE.getEvidenceElement_Event());
				basePart.initEvent(eventSettings);
			}
			if (isAccessible(EvidenceViewsRepository.Refutes.Properties.attribute)) {
				attributeSettings = new ReferencesTableSettings(refutes, EvidencePackage.eINSTANCE.getEvidenceEvaluation_Attribute());
				basePart.initAttribute(attributeSettings);
			}
			if (isAccessible(EvidenceViewsRepository.Refutes.Properties.element)) {
				// init part
				elementSettings = new EObjectFlatComboSettings(refutes, EvidencePackage.eINSTANCE.getRefutes_Element());
	...
	public EStructuralFeature associatedFeature(Object editorKey) {
		if (editorKey == EvidenceViewsRepository.Refutes.Properties.taggedValue) {
			return SACMPackage.eINSTANCE.getModelElement_TaggedValue();
		}
		if (editorKey == EvidenceViewsRepository.Refutes.Properties.annotation) {
			return SACMPackage.eINSTANCE.getModelElement_Annotation();
		}
		if (editorKey == EvidenceViewsRepository.Refutes.Properties.id) {
			return SACMPackage.eINSTANCE.getModelElement_Id();
		}
		if (editorKey == EvidenceViewsRepository.Refutes.Properties.timing) {
			return EvidencePackage.eINSTANCE.getEvidenceElement_Timing();
		}
		if (editorKey == EvidenceViewsRepository.Refutes.Properties.custody) {
			return EvidencePackage.eINSTANCE.getEvidenceElement_Custody();
		}
		if (editorKey == EvidenceViewsRepository.Refutes.Properties.provenance) {
			return EvidencePackage.eINSTANCE.getEvidenceElement_Provenance();
		}
		if (editorKey == EvidenceViewsRepository.Refutes.Properties.event) {
			return EvidencePackage.eINSTANCE.getEvidenceElement_Event();
		}
		if (editorKey == EvidenceViewsRepository.Refutes.Properties.attribute) {
			return EvidencePackage.eINSTANCE.getEvidenceEvaluation_Attribute();
		}
		if (editorKey == EvidenceViewsRepository.Refutes.Properties.element) {
			return EvidencePackage.eINSTANCE.getRefutes_Element();
		}
	...
	public void updateSemanticModel(final IPropertiesEditionEvent event) {
		Refutes refutes = (Refutes)semanticObject;
		if (EvidenceViewsRepository.Refutes.Properties.taggedValue == event.getAffectedEditor()) {
		...
		if (EvidenceViewsRepository.Refutes.Properties.annotation == event.getAffectedEditor()) {
			if (event.getKind() == PropertiesEditionEvent.ADD) {
		...
		if (EvidenceViewsRepository.Refutes.Properties.id == event.getAffectedEditor()) {
			refutes.setId((java.lang.String)EEFConverterUtil.createFromString(SACMPackage.Literals.STRING, (String)event.getNewValue()));
		}
		if (EvidenceViewsRepository.Refutes.Properties.timing == event.getAffectedEditor()) {
			if (event.getKind() == PropertiesEditionEvent.ADD) {
		...
		if (EvidenceViewsRepository.Refutes.Properties.custody == event.getAffectedEditor()) {
			if (event.getKind() == PropertiesEditionEvent.ADD) {
		...
		if (EvidenceViewsRepository.Refutes.Properties.provenance == event.getAffectedEditor()) {
			if (event.getKind() == PropertiesEditionEvent.ADD) {
		...
		if (EvidenceViewsRepository.Refutes.Properties.event == event.getAffectedEditor()) {
			if (event.getKind() == PropertiesEditionEvent.ADD) {
		...
		if (EvidenceViewsRepository.Refutes.Properties.attribute == event.getAffectedEditor()) {
			if (event.getKind() == PropertiesEditionEvent.ADD) {
		...
		if (EvidenceViewsRepository.Refutes.Properties.element == event.getAffectedEditor()) {
			if (event.getKind() == PropertiesEditionEvent.SET) {
				elementSettings.setToReference((FormalAssertion)event.getNewValue());
	...
	public void updatePart(Notification msg) {
		...
		if (editingPart.isVisible()) {
			RefutesPropertiesEditionPart basePart = (RefutesPropertiesEditionPart)editingPart;
			if (SACMPackage.eINSTANCE.getModelElement_TaggedValue().equals(msg.getFeature()) && isAccessible(EvidenceViewsRepository.Refutes.Properties.taggedValue))
				basePart.updateTaggedValue();
			if (SACMPackage.eINSTANCE.getModelElement_Annotation().equals(msg.getFeature()) && isAccessible(EvidenceViewsRepository.Refutes.Properties.annotation))
				basePart.updateAnnotation();
			if (SACMPackage.eINSTANCE.getModelElement_Id().equals(msg.getFeature()) && msg.getNotifier().equals(semanticObject) && basePart != null && isAccessible(EvidenceViewsRepository.Refutes.Properties.id)) {
			...
			if (EvidencePackage.eINSTANCE.getEvidenceElement_Timing().equals(msg.getFeature()) && isAccessible(EvidenceViewsRepository.Refutes.Properties.timing))
				basePart.updateTiming();
			if (EvidencePackage.eINSTANCE.getEvidenceElement_Custody().equals(msg.getFeature()) && isAccessible(EvidenceViewsRepository.Refutes.Properties.custody))
				basePart.updateCustody();
			if (EvidencePackage.eINSTANCE.getEvidenceElement_Provenance().equals(msg.getFeature()) && isAccessible(EvidenceViewsRepository.Refutes.Properties.provenance))
				basePart.updateProvenance();
			if (EvidencePackage.eINSTANCE.getEvidenceElement_Event().equals(msg.getFeature()) && isAccessible(EvidenceViewsRepository.Refutes.Properties.event))
				basePart.updateEvent();
			if (EvidencePackage.eINSTANCE.getEvidenceEvaluation_Attribute().equals(msg.getFeature()) && isAccessible(EvidenceViewsRepository.Refutes.Properties.attribute))
				basePart.updateAttribute();
			if (EvidencePackage.eINSTANCE.getRefutes_Element().equals(msg.getFeature()) && basePart != null && isAccessible(EvidenceViewsRepository.Refutes.Properties.element))
	...
	protected NotificationFilter[] getNotificationFilters() {
		NotificationFilter filter = new EStructuralFeatureNotificationFilter(
			...
			EvidencePackage.eINSTANCE.getRefutes_Element()		);
	...
	public boolean isRequired(Object key, int kind) {
		return key == EvidenceViewsRepository.Refutes.Properties.id || key == EvidenceViewsRepository.Refutes.Properties.element;
	}
