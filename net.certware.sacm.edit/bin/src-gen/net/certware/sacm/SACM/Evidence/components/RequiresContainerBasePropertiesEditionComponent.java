 ...
 * 
 */
public class RequiresContainerBasePropertiesEditionComponent extends SinglePartPropertiesEditingComponent {
	 ...
	 * 
	 */
	public RequiresContainerBasePropertiesEditionComponent(PropertiesEditingContext editingContext, EObject requiresContainer, String editing_mode) {
		super(editingContext, requiresContainer, editing_mode);
		parts = new String[] { BASE_PART };
		repositoryKey = EvidenceViewsRepository.class;
		partKey = EvidenceViewsRepository.RequiresContainer.class;
	}
	...
	public void initPart(Object key, int kind, EObject elt, ResourceSet allResource) {
		...
		if (editingPart != null && key == partKey) {
			...
			final RequiresContainer requiresContainer = (RequiresContainer)elt;
			final RequiresContainerPropertiesEditionPart basePart = (RequiresContainerPropertiesEditionPart)editingPart;
			// init values
			if (isAccessible(EvidenceViewsRepository.RequiresContainer.Properties.taggedValue)) {
				taggedValueSettings = new ReferencesTableSettings(requiresContainer, SACMPackage.eINSTANCE.getModelElement_TaggedValue());
				basePart.initTaggedValue(taggedValueSettings);
			}
			if (isAccessible(EvidenceViewsRepository.RequiresContainer.Properties.annotation)) {
				annotationSettings = new ReferencesTableSettings(requiresContainer, SACMPackage.eINSTANCE.getModelElement_Annotation());
				basePart.initAnnotation(annotationSettings);
			}
			if (isAccessible(EvidenceViewsRepository.RequiresContainer.Properties.id))
				basePart.setId(EEFConverterUtil.convertToString(SACMPackage.Literals.STRING, requiresContainer.getId()));
			
			if (isAccessible(EvidenceViewsRepository.RequiresContainer.Properties.timing)) {
				timingSettings = new ReferencesTableSettings(requiresContainer, EvidencePackage.eINSTANCE.getEvidenceElement_Timing());
				basePart.initTiming(timingSettings);
			}
			if (isAccessible(EvidenceViewsRepository.RequiresContainer.Properties.custody)) {
				custodySettings = new ReferencesTableSettings(requiresContainer, EvidencePackage.eINSTANCE.getEvidenceElement_Custody());
				basePart.initCustody(custodySettings);
			}
			if (isAccessible(EvidenceViewsRepository.RequiresContainer.Properties.provenance)) {
				provenanceSettings = new ReferencesTableSettings(requiresContainer, EvidencePackage.eINSTANCE.getEvidenceElement_Provenance());
				basePart.initProvenance(provenanceSettings);
			}
			if (isAccessible(EvidenceViewsRepository.RequiresContainer.Properties.event)) {
				eventSettings = new ReferencesTableSettings(requiresContainer, EvidencePackage.eINSTANCE.getEvidenceElement_Event());
				basePart.initEvent(eventSettings);
			}
			if (isAccessible(EvidenceViewsRepository.RequiresContainer.Properties.container)) {
				// init part
				containerSettings = new EObjectFlatComboSettings(requiresContainer, EvidencePackage.eINSTANCE.getRequiresContainer_Container());
			...
			if (isAccessible(EvidenceViewsRepository.RequiresContainer.Properties.container)) {
				basePart.addFilterToContainer(new ViewerFilter() {
					 ...
					 */
					public boolean select(Viewer viewer, Object parentElement, Object element) {
						return (element instanceof EvidenceContainer);
					}
	...
	public EStructuralFeature associatedFeature(Object editorKey) {
		if (editorKey == EvidenceViewsRepository.RequiresContainer.Properties.taggedValue) {
			return SACMPackage.eINSTANCE.getModelElement_TaggedValue();
		}
		if (editorKey == EvidenceViewsRepository.RequiresContainer.Properties.annotation) {
			return SACMPackage.eINSTANCE.getModelElement_Annotation();
		}
		if (editorKey == EvidenceViewsRepository.RequiresContainer.Properties.id) {
			return SACMPackage.eINSTANCE.getModelElement_Id();
		}
		if (editorKey == EvidenceViewsRepository.RequiresContainer.Properties.timing) {
			return EvidencePackage.eINSTANCE.getEvidenceElement_Timing();
		}
		if (editorKey == EvidenceViewsRepository.RequiresContainer.Properties.custody) {
			return EvidencePackage.eINSTANCE.getEvidenceElement_Custody();
		}
		if (editorKey == EvidenceViewsRepository.RequiresContainer.Properties.provenance) {
			return EvidencePackage.eINSTANCE.getEvidenceElement_Provenance();
		}
		if (editorKey == EvidenceViewsRepository.RequiresContainer.Properties.event) {
			return EvidencePackage.eINSTANCE.getEvidenceElement_Event();
		}
		if (editorKey == EvidenceViewsRepository.RequiresContainer.Properties.container) {
			return EvidencePackage.eINSTANCE.getRequiresContainer_Container();
		}
	...
	public void updateSemanticModel(final IPropertiesEditionEvent event) {
		RequiresContainer requiresContainer = (RequiresContainer)semanticObject;
		if (EvidenceViewsRepository.RequiresContainer.Properties.taggedValue == event.getAffectedEditor()) {
		...
		if (EvidenceViewsRepository.RequiresContainer.Properties.annotation == event.getAffectedEditor()) {
			if (event.getKind() == PropertiesEditionEvent.ADD) {
		...
		if (EvidenceViewsRepository.RequiresContainer.Properties.id == event.getAffectedEditor()) {
			requiresContainer.setId((java.lang.String)EEFConverterUtil.createFromString(SACMPackage.Literals.STRING, (String)event.getNewValue()));
		}
		if (EvidenceViewsRepository.RequiresContainer.Properties.timing == event.getAffectedEditor()) {
			if (event.getKind() == PropertiesEditionEvent.ADD) {
		...
		if (EvidenceViewsRepository.RequiresContainer.Properties.custody == event.getAffectedEditor()) {
			if (event.getKind() == PropertiesEditionEvent.ADD) {
		...
		if (EvidenceViewsRepository.RequiresContainer.Properties.provenance == event.getAffectedEditor()) {
			if (event.getKind() == PropertiesEditionEvent.ADD) {
		...
		if (EvidenceViewsRepository.RequiresContainer.Properties.event == event.getAffectedEditor()) {
			if (event.getKind() == PropertiesEditionEvent.ADD) {
		...
		if (EvidenceViewsRepository.RequiresContainer.Properties.container == event.getAffectedEditor()) {
			if (event.getKind() == PropertiesEditionEvent.SET) {
				containerSettings.setToReference((EvidenceContainer)event.getNewValue());
			} else if (event.getKind() == PropertiesEditionEvent.ADD) {
				EvidenceContainer eObject = EvidenceFactory.eINSTANCE.createEvidenceContainer();
	...
	public void updatePart(Notification msg) {
		...
		if (editingPart.isVisible()) {
			RequiresContainerPropertiesEditionPart basePart = (RequiresContainerPropertiesEditionPart)editingPart;
			if (SACMPackage.eINSTANCE.getModelElement_TaggedValue().equals(msg.getFeature()) && isAccessible(EvidenceViewsRepository.RequiresContainer.Properties.taggedValue))
				basePart.updateTaggedValue();
			if (SACMPackage.eINSTANCE.getModelElement_Annotation().equals(msg.getFeature()) && isAccessible(EvidenceViewsRepository.RequiresContainer.Properties.annotation))
				basePart.updateAnnotation();
			if (SACMPackage.eINSTANCE.getModelElement_Id().equals(msg.getFeature()) && msg.getNotifier().equals(semanticObject) && basePart != null && isAccessible(EvidenceViewsRepository.RequiresContainer.Properties.id)) {
			...
			if (EvidencePackage.eINSTANCE.getEvidenceElement_Timing().equals(msg.getFeature()) && isAccessible(EvidenceViewsRepository.RequiresContainer.Properties.timing))
				basePart.updateTiming();
			if (EvidencePackage.eINSTANCE.getEvidenceElement_Custody().equals(msg.getFeature()) && isAccessible(EvidenceViewsRepository.RequiresContainer.Properties.custody))
				basePart.updateCustody();
			if (EvidencePackage.eINSTANCE.getEvidenceElement_Provenance().equals(msg.getFeature()) && isAccessible(EvidenceViewsRepository.RequiresContainer.Properties.provenance))
				basePart.updateProvenance();
			if (EvidencePackage.eINSTANCE.getEvidenceElement_Event().equals(msg.getFeature()) && isAccessible(EvidenceViewsRepository.RequiresContainer.Properties.event))
				basePart.updateEvent();
			if (EvidencePackage.eINSTANCE.getRequiresContainer_Container().equals(msg.getFeature()) && basePart != null && isAccessible(EvidenceViewsRepository.RequiresContainer.Properties.container))
	...
	protected NotificationFilter[] getNotificationFilters() {
		NotificationFilter filter = new EStructuralFeatureNotificationFilter(
			...
			EvidencePackage.eINSTANCE.getRequiresContainer_Container()		);
	...
	public boolean isRequired(Object key, int kind) {
		return key == EvidenceViewsRepository.RequiresContainer.Properties.id || key == EvidenceViewsRepository.RequiresContainer.Properties.container;
	}
