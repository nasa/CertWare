 ...
 * 
 */
public class ResolvesBasePropertiesEditionComponent extends SinglePartPropertiesEditingComponent {
	...
	/**
	 * Settings for element EObjectFlatComboViewer
	 */
	private EObjectFlatComboSettings elementSettings;
	 ...
	 * 
	 */
	public ResolvesBasePropertiesEditionComponent(PropertiesEditingContext editingContext, EObject resolves, String editing_mode) {
		super(editingContext, resolves, editing_mode);
		parts = new String[] { BASE_PART };
		repositoryKey = EvidenceViewsRepository.class;
		partKey = EvidenceViewsRepository.Resolves.class;
	}
	...
	public void initPart(Object key, int kind, EObject elt, ResourceSet allResource) {
		...
		if (editingPart != null && key == partKey) {
			...
			final Resolves resolves = (Resolves)elt;
			final ResolvesPropertiesEditionPart basePart = (ResolvesPropertiesEditionPart)editingPart;
			// init values
			if (isAccessible(EvidenceViewsRepository.Resolves.Properties.taggedValue)) {
				taggedValueSettings = new ReferencesTableSettings(resolves, SACMPackage.eINSTANCE.getModelElement_TaggedValue());
				basePart.initTaggedValue(taggedValueSettings);
			}
			if (isAccessible(EvidenceViewsRepository.Resolves.Properties.annotation)) {
				annotationSettings = new ReferencesTableSettings(resolves, SACMPackage.eINSTANCE.getModelElement_Annotation());
				basePart.initAnnotation(annotationSettings);
			}
			if (isAccessible(EvidenceViewsRepository.Resolves.Properties.id))
				basePart.setId(EEFConverterUtil.convertToString(SACMPackage.Literals.STRING, resolves.getId()));
			
			if (isAccessible(EvidenceViewsRepository.Resolves.Properties.timing)) {
				timingSettings = new ReferencesTableSettings(resolves, EvidencePackage.eINSTANCE.getEvidenceElement_Timing());
				basePart.initTiming(timingSettings);
			}
			if (isAccessible(EvidenceViewsRepository.Resolves.Properties.custody)) {
				custodySettings = new ReferencesTableSettings(resolves, EvidencePackage.eINSTANCE.getEvidenceElement_Custody());
				basePart.initCustody(custodySettings);
			}
			if (isAccessible(EvidenceViewsRepository.Resolves.Properties.provenance)) {
				provenanceSettings = new ReferencesTableSettings(resolves, EvidencePackage.eINSTANCE.getEvidenceElement_Provenance());
				basePart.initProvenance(provenanceSettings);
			}
			if (isAccessible(EvidenceViewsRepository.Resolves.Properties.event)) {
				eventSettings = new ReferencesTableSettings(resolves, EvidencePackage.eINSTANCE.getEvidenceElement_Event());
				basePart.initEvent(eventSettings);
			}
			if (isAccessible(EvidenceViewsRepository.Resolves.Properties.attribute)) {
				attributeSettings = new ReferencesTableSettings(resolves, EvidencePackage.eINSTANCE.getEvidenceEvaluation_Attribute());
				basePart.initAttribute(attributeSettings);
			}
			if (isAccessible(EvidenceViewsRepository.Resolves.Properties.element)) {
				// init part
				elementSettings = new EObjectFlatComboSettings(resolves, EvidencePackage.eINSTANCE.getResolves_Element());
				basePart.initElement(elementSettings);
				// set the button mode
				basePart.setElementButtonMode(ButtonsModeEnum.BROWSE);
			}
			// init filters
			if (isAccessible(EvidenceViewsRepository.Resolves.Properties.taggedValue)) {
			...
			if (isAccessible(EvidenceViewsRepository.Resolves.Properties.element)) {
				basePart.addFilterToElement(new ViewerFilter() {
					 ...
					 */
					public boolean select(Viewer viewer, Object parentElement, Object element) {
						return (element instanceof EvidenceObservation);
					}
	...
	public EStructuralFeature associatedFeature(Object editorKey) {
		if (editorKey == EvidenceViewsRepository.Resolves.Properties.taggedValue) {
			return SACMPackage.eINSTANCE.getModelElement_TaggedValue();
		}
		if (editorKey == EvidenceViewsRepository.Resolves.Properties.annotation) {
			return SACMPackage.eINSTANCE.getModelElement_Annotation();
		}
		if (editorKey == EvidenceViewsRepository.Resolves.Properties.id) {
			return SACMPackage.eINSTANCE.getModelElement_Id();
		}
		if (editorKey == EvidenceViewsRepository.Resolves.Properties.timing) {
			return EvidencePackage.eINSTANCE.getEvidenceElement_Timing();
		}
		if (editorKey == EvidenceViewsRepository.Resolves.Properties.custody) {
			return EvidencePackage.eINSTANCE.getEvidenceElement_Custody();
		}
		if (editorKey == EvidenceViewsRepository.Resolves.Properties.provenance) {
			return EvidencePackage.eINSTANCE.getEvidenceElement_Provenance();
		}
		if (editorKey == EvidenceViewsRepository.Resolves.Properties.event) {
			return EvidencePackage.eINSTANCE.getEvidenceElement_Event();
		}
		if (editorKey == EvidenceViewsRepository.Resolves.Properties.attribute) {
			return EvidencePackage.eINSTANCE.getEvidenceEvaluation_Attribute();
		}
		if (editorKey == EvidenceViewsRepository.Resolves.Properties.element) {
			return EvidencePackage.eINSTANCE.getResolves_Element();
		}
	...
	public void updateSemanticModel(final IPropertiesEditionEvent event) {
		Resolves resolves = (Resolves)semanticObject;
		if (EvidenceViewsRepository.Resolves.Properties.taggedValue == event.getAffectedEditor()) {
		...
		if (EvidenceViewsRepository.Resolves.Properties.annotation == event.getAffectedEditor()) {
			if (event.getKind() == PropertiesEditionEvent.ADD) {
		...
		if (EvidenceViewsRepository.Resolves.Properties.id == event.getAffectedEditor()) {
			resolves.setId((java.lang.String)EEFConverterUtil.createFromString(SACMPackage.Literals.STRING, (String)event.getNewValue()));
		}
		if (EvidenceViewsRepository.Resolves.Properties.timing == event.getAffectedEditor()) {
			if (event.getKind() == PropertiesEditionEvent.ADD) {
		...
		if (EvidenceViewsRepository.Resolves.Properties.custody == event.getAffectedEditor()) {
			if (event.getKind() == PropertiesEditionEvent.ADD) {
		...
		if (EvidenceViewsRepository.Resolves.Properties.provenance == event.getAffectedEditor()) {
			if (event.getKind() == PropertiesEditionEvent.ADD) {
		...
		if (EvidenceViewsRepository.Resolves.Properties.event == event.getAffectedEditor()) {
			if (event.getKind() == PropertiesEditionEvent.ADD) {
		...
		if (EvidenceViewsRepository.Resolves.Properties.attribute == event.getAffectedEditor()) {
			if (event.getKind() == PropertiesEditionEvent.ADD) {
		...
		if (EvidenceViewsRepository.Resolves.Properties.element == event.getAffectedEditor()) {
			if (event.getKind() == PropertiesEditionEvent.SET) {
				elementSettings.setToReference((EvidenceObservation)event.getNewValue());
	...
	public void updatePart(Notification msg) {
		...
		if (editingPart.isVisible()) {
			ResolvesPropertiesEditionPart basePart = (ResolvesPropertiesEditionPart)editingPart;
			if (SACMPackage.eINSTANCE.getModelElement_TaggedValue().equals(msg.getFeature()) && isAccessible(EvidenceViewsRepository.Resolves.Properties.taggedValue))
				basePart.updateTaggedValue();
			if (SACMPackage.eINSTANCE.getModelElement_Annotation().equals(msg.getFeature()) && isAccessible(EvidenceViewsRepository.Resolves.Properties.annotation))
				basePart.updateAnnotation();
			if (SACMPackage.eINSTANCE.getModelElement_Id().equals(msg.getFeature()) && msg.getNotifier().equals(semanticObject) && basePart != null && isAccessible(EvidenceViewsRepository.Resolves.Properties.id)) {
			...
			if (EvidencePackage.eINSTANCE.getEvidenceElement_Timing().equals(msg.getFeature()) && isAccessible(EvidenceViewsRepository.Resolves.Properties.timing))
				basePart.updateTiming();
			if (EvidencePackage.eINSTANCE.getEvidenceElement_Custody().equals(msg.getFeature()) && isAccessible(EvidenceViewsRepository.Resolves.Properties.custody))
				basePart.updateCustody();
			if (EvidencePackage.eINSTANCE.getEvidenceElement_Provenance().equals(msg.getFeature()) && isAccessible(EvidenceViewsRepository.Resolves.Properties.provenance))
				basePart.updateProvenance();
			if (EvidencePackage.eINSTANCE.getEvidenceElement_Event().equals(msg.getFeature()) && isAccessible(EvidenceViewsRepository.Resolves.Properties.event))
				basePart.updateEvent();
			if (EvidencePackage.eINSTANCE.getEvidenceEvaluation_Attribute().equals(msg.getFeature()) && isAccessible(EvidenceViewsRepository.Resolves.Properties.attribute))
				basePart.updateAttribute();
			if (EvidencePackage.eINSTANCE.getResolves_Element().equals(msg.getFeature()) && basePart != null && isAccessible(EvidenceViewsRepository.Resolves.Properties.element))
				basePart.setElement((EObject)msg.getNewValue());
	...
	protected NotificationFilter[] getNotificationFilters() {
		NotificationFilter filter = new EStructuralFeatureNotificationFilter(
			...
			EvidencePackage.eINSTANCE.getResolves_Element()		);
	...
	public boolean isRequired(Object key, int kind) {
		return key == EvidenceViewsRepository.Resolves.Properties.id || key == EvidenceViewsRepository.Resolves.Properties.element;
	}
