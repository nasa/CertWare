 ...
 * 
 */
public class SupportsBasePropertiesEditionComponent extends SinglePartPropertiesEditingComponent {
	 ...
	 * 
	 */
	public SupportsBasePropertiesEditionComponent(PropertiesEditingContext editingContext, EObject supports, String editing_mode) {
		super(editingContext, supports, editing_mode);
		parts = new String[] { BASE_PART };
		repositoryKey = EvidenceViewsRepository.class;
		partKey = EvidenceViewsRepository.Supports.class;
	}
	...
	public void initPart(Object key, int kind, EObject elt, ResourceSet allResource) {
		...
		if (editingPart != null && key == partKey) {
			...
			final Supports supports = (Supports)elt;
			final SupportsPropertiesEditionPart basePart = (SupportsPropertiesEditionPart)editingPart;
			// init values
			if (isAccessible(EvidenceViewsRepository.Supports.Properties.taggedValue)) {
				taggedValueSettings = new ReferencesTableSettings(supports, SACMPackage.eINSTANCE.getModelElement_TaggedValue());
				basePart.initTaggedValue(taggedValueSettings);
			}
			if (isAccessible(EvidenceViewsRepository.Supports.Properties.annotation)) {
				annotationSettings = new ReferencesTableSettings(supports, SACMPackage.eINSTANCE.getModelElement_Annotation());
				basePart.initAnnotation(annotationSettings);
			}
			if (isAccessible(EvidenceViewsRepository.Supports.Properties.id))
				basePart.setId(EEFConverterUtil.convertToString(SACMPackage.Literals.STRING, supports.getId()));
			
			if (isAccessible(EvidenceViewsRepository.Supports.Properties.timing)) {
				timingSettings = new ReferencesTableSettings(supports, EvidencePackage.eINSTANCE.getEvidenceElement_Timing());
				basePart.initTiming(timingSettings);
			}
			if (isAccessible(EvidenceViewsRepository.Supports.Properties.custody)) {
				custodySettings = new ReferencesTableSettings(supports, EvidencePackage.eINSTANCE.getEvidenceElement_Custody());
				basePart.initCustody(custodySettings);
			}
			if (isAccessible(EvidenceViewsRepository.Supports.Properties.provenance)) {
				provenanceSettings = new ReferencesTableSettings(supports, EvidencePackage.eINSTANCE.getEvidenceElement_Provenance());
				basePart.initProvenance(provenanceSettings);
			}
			if (isAccessible(EvidenceViewsRepository.Supports.Properties.event)) {
				eventSettings = new ReferencesTableSettings(supports, EvidencePackage.eINSTANCE.getEvidenceElement_Event());
				basePart.initEvent(eventSettings);
			}
			if (isAccessible(EvidenceViewsRepository.Supports.Properties.attribute)) {
				attributeSettings = new ReferencesTableSettings(supports, EvidencePackage.eINSTANCE.getEvidenceEvaluation_Attribute());
				basePart.initAttribute(attributeSettings);
			}
			if (isAccessible(EvidenceViewsRepository.Supports.Properties.assertion)) {
				// init part
				assertionSettings = new EObjectFlatComboSettings(supports, EvidencePackage.eINSTANCE.getEvidenceRelation_Assertion());
				basePart.initAssertion(assertionSettings);
				// set the button mode
				basePart.setAssertionButtonMode(ButtonsModeEnum.BROWSE);
			}
			if (isAccessible(EvidenceViewsRepository.Supports.Properties.subject)) {
				// init part
				subjectSettings = new EObjectFlatComboSettings(supports, EvidencePackage.eINSTANCE.getEvidenceRelation_Subject());
	...
	public EStructuralFeature associatedFeature(Object editorKey) {
		if (editorKey == EvidenceViewsRepository.Supports.Properties.taggedValue) {
			return SACMPackage.eINSTANCE.getModelElement_TaggedValue();
		}
		if (editorKey == EvidenceViewsRepository.Supports.Properties.annotation) {
			return SACMPackage.eINSTANCE.getModelElement_Annotation();
		}
		if (editorKey == EvidenceViewsRepository.Supports.Properties.id) {
			return SACMPackage.eINSTANCE.getModelElement_Id();
		}
		if (editorKey == EvidenceViewsRepository.Supports.Properties.timing) {
			return EvidencePackage.eINSTANCE.getEvidenceElement_Timing();
		}
		if (editorKey == EvidenceViewsRepository.Supports.Properties.custody) {
			return EvidencePackage.eINSTANCE.getEvidenceElement_Custody();
		}
		if (editorKey == EvidenceViewsRepository.Supports.Properties.provenance) {
			return EvidencePackage.eINSTANCE.getEvidenceElement_Provenance();
		}
		if (editorKey == EvidenceViewsRepository.Supports.Properties.event) {
			return EvidencePackage.eINSTANCE.getEvidenceElement_Event();
		}
		if (editorKey == EvidenceViewsRepository.Supports.Properties.attribute) {
			return EvidencePackage.eINSTANCE.getEvidenceEvaluation_Attribute();
		}
		if (editorKey == EvidenceViewsRepository.Supports.Properties.assertion) {
			return EvidencePackage.eINSTANCE.getEvidenceRelation_Assertion();
		}
		if (editorKey == EvidenceViewsRepository.Supports.Properties.subject) {
	...
	public void updateSemanticModel(final IPropertiesEditionEvent event) {
		Supports supports = (Supports)semanticObject;
		if (EvidenceViewsRepository.Supports.Properties.taggedValue == event.getAffectedEditor()) {
		...
		if (EvidenceViewsRepository.Supports.Properties.annotation == event.getAffectedEditor()) {
			if (event.getKind() == PropertiesEditionEvent.ADD) {
		...
		if (EvidenceViewsRepository.Supports.Properties.id == event.getAffectedEditor()) {
			supports.setId((java.lang.String)EEFConverterUtil.createFromString(SACMPackage.Literals.STRING, (String)event.getNewValue()));
		}
		if (EvidenceViewsRepository.Supports.Properties.timing == event.getAffectedEditor()) {
			if (event.getKind() == PropertiesEditionEvent.ADD) {
		...
		if (EvidenceViewsRepository.Supports.Properties.custody == event.getAffectedEditor()) {
			if (event.getKind() == PropertiesEditionEvent.ADD) {
		...
		if (EvidenceViewsRepository.Supports.Properties.provenance == event.getAffectedEditor()) {
			if (event.getKind() == PropertiesEditionEvent.ADD) {
		...
		if (EvidenceViewsRepository.Supports.Properties.event == event.getAffectedEditor()) {
			if (event.getKind() == PropertiesEditionEvent.ADD) {
		...
		if (EvidenceViewsRepository.Supports.Properties.attribute == event.getAffectedEditor()) {
			if (event.getKind() == PropertiesEditionEvent.ADD) {
		...
		if (EvidenceViewsRepository.Supports.Properties.assertion == event.getAffectedEditor()) {
		...
		if (EvidenceViewsRepository.Supports.Properties.subject == event.getAffectedEditor()) {
			if (event.getKind() == PropertiesEditionEvent.SET) {
	...
	public void updatePart(Notification msg) {
		...
		if (editingPart.isVisible()) {
			SupportsPropertiesEditionPart basePart = (SupportsPropertiesEditionPart)editingPart;
			if (SACMPackage.eINSTANCE.getModelElement_TaggedValue().equals(msg.getFeature()) && isAccessible(EvidenceViewsRepository.Supports.Properties.taggedValue))
				basePart.updateTaggedValue();
			if (SACMPackage.eINSTANCE.getModelElement_Annotation().equals(msg.getFeature()) && isAccessible(EvidenceViewsRepository.Supports.Properties.annotation))
				basePart.updateAnnotation();
			if (SACMPackage.eINSTANCE.getModelElement_Id().equals(msg.getFeature()) && msg.getNotifier().equals(semanticObject) && basePart != null && isAccessible(EvidenceViewsRepository.Supports.Properties.id)) {
			...
			if (EvidencePackage.eINSTANCE.getEvidenceElement_Timing().equals(msg.getFeature()) && isAccessible(EvidenceViewsRepository.Supports.Properties.timing))
				basePart.updateTiming();
			if (EvidencePackage.eINSTANCE.getEvidenceElement_Custody().equals(msg.getFeature()) && isAccessible(EvidenceViewsRepository.Supports.Properties.custody))
				basePart.updateCustody();
			if (EvidencePackage.eINSTANCE.getEvidenceElement_Provenance().equals(msg.getFeature()) && isAccessible(EvidenceViewsRepository.Supports.Properties.provenance))
				basePart.updateProvenance();
			if (EvidencePackage.eINSTANCE.getEvidenceElement_Event().equals(msg.getFeature()) && isAccessible(EvidenceViewsRepository.Supports.Properties.event))
				basePart.updateEvent();
			if (EvidencePackage.eINSTANCE.getEvidenceEvaluation_Attribute().equals(msg.getFeature()) && isAccessible(EvidenceViewsRepository.Supports.Properties.attribute))
				basePart.updateAttribute();
			if (EvidencePackage.eINSTANCE.getEvidenceRelation_Assertion().equals(msg.getFeature()) && basePart != null && isAccessible(EvidenceViewsRepository.Supports.Properties.assertion))
				basePart.setAssertion((EObject)msg.getNewValue());
			if (EvidencePackage.eINSTANCE.getEvidenceRelation_Subject().equals(msg.getFeature()) && basePart != null && isAccessible(EvidenceViewsRepository.Supports.Properties.subject))
	...
	public boolean isRequired(Object key, int kind) {
		return key == EvidenceViewsRepository.Supports.Properties.id || key == EvidenceViewsRepository.Supports.Properties.assertion || key == EvidenceViewsRepository.Supports.Properties.subject;
	}
