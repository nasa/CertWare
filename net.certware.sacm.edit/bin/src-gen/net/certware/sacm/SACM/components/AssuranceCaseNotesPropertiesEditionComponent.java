 ...
 * 
 */
public class AssuranceCaseNotesPropertiesEditionComponent extends SinglePartPropertiesEditingComponent {
	 ...
	 * 
	 */
	public AssuranceCaseNotesPropertiesEditionComponent(PropertiesEditingContext editingContext, EObject assuranceCase, String editing_mode) {
		super(editingContext, assuranceCase, editing_mode);
		parts = new String[] { NOTES_PART };
		repositoryKey = SACMViewsRepository.class;
		partKey = SACMViewsRepository.Notes.class;
	}
	...
	public void initPart(Object key, int kind, EObject elt, ResourceSet allResource) {
		...
		if (editingPart != null && key == partKey) {
			...
			// init values
			if (isAccessible(SACMViewsRepository.Notes.Properties.taggedValue)) {
				taggedValueSettings = new ReferencesTableSettings(assuranceCase, SACMPackage.eINSTANCE.getModelElement_TaggedValue());
				notesPart.initTaggedValue(taggedValueSettings);
			}
			if (isAccessible(SACMViewsRepository.Notes.Properties.annotation)) {
				annotationSettings = new ReferencesTableSettings(assuranceCase, SACMPackage.eINSTANCE.getModelElement_Annotation());
	...
	public EStructuralFeature associatedFeature(Object editorKey) {
		if (editorKey == SACMViewsRepository.Notes.Properties.taggedValue) {
			return SACMPackage.eINSTANCE.getModelElement_TaggedValue();
		}
		if (editorKey == SACMViewsRepository.Notes.Properties.annotation) {
	...
	public void updateSemanticModel(final IPropertiesEditionEvent event) {
		...
		if (SACMViewsRepository.Notes.Properties.taggedValue == event.getAffectedEditor()) {
		...
		if (SACMViewsRepository.Notes.Properties.annotation == event.getAffectedEditor()) {
			if (event.getKind() == PropertiesEditionEvent.ADD) {
	...
	public void updatePart(Notification msg) {
		...
		if (editingPart.isVisible()) {
			...
			if (SACMPackage.eINSTANCE.getModelElement_TaggedValue().equals(msg.getFeature()) && isAccessible(SACMViewsRepository.Notes.Properties.taggedValue))
				notesPart.updateTaggedValue();
			if (SACMPackage.eINSTANCE.getModelElement_Annotation().equals(msg.getFeature()) && isAccessible(SACMViewsRepository.Notes.Properties.annotation))
